...
private:
  ...
  bool mLastButtonState =
      false; /**< Determines was the user rotating the camera last frame. */
  ...
  static const float ACCELERATION; /**< Acceleration that determines how quickly
                                      to go from starting to top speed. */
  static const float
      FAST_MODE_MULTIPLIER; /**< Multiplier applied to the speed when the fast
                               move button is held. */
  static const float
      ROTATION_SPEED; /**< Determines speed of camera rotation. */
};

using HCameraFlyer = GameObjectHandle<CameraFlyer>;
