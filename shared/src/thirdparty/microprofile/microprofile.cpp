...
void MicroProfileGpuSetCallbacks(
    ...
    MicroProfileGpuFlip_CB Flip, MicroProfileGpuShutdown_CB Shutdown) {
...
static_assert(0 == (MICROPROFILE_MAX_GROUPS % 32),
              "MICROPROFILE_MAX_GROUPS must be divisible by 32");
...
#define MICROPROFILE_ALLOC(nSize, nAlign)                                      \
  MicroProfileAllocAligned(nSize, nAlign);
...
template <typename T> T MicroProfileMin(T a, T b);
template <typename T> T MicroProfileMax(T a, T b);
template <typename T> T MicroProfileClamp(T a, T min_, T max_);
...
MicroProfileLogEntry MicroProfileMakeLogIndex(uint64_t nBegin,
                                              MicroProfileToken nToken,
                                              int64_t nTick);
int64_t MicroProfileLogTickDifference(MicroProfileLogEntry Start,
                                      MicroProfileLogEntry End);
...
MicroProfileToken MicroProfileMakeToken(uint64_t nGroupMask,
                                        uint32_t nGroupIndex, uint16_t nTimer);
...
void MicroProfileFreeAligned(void *pMem) { free(pMem); }
...
void MicroProfileFreeAligned(void *pMem) { _aligned_free(pMem); }
...
#include <time.h>
#include <unistd.h>
inline int64_t MicroProfileTicksPerSecondCpu_() { return 1000000000ll; }
...
void MicroProfileInstrumentFunction(void *pFunction, const char *pModuleName,
                                    const char *pFunctionName, uint32_t nColor);
...
void MicroProfileInstrumentWithoutSymbols(const char **pModules,
                                          const char **pSymbols,
...
#if MICROPROFILE_COUNTER_HISTORY // uses 1kb per allocated counter. 512kb for
                                 // default counter count
  uint32_t nCounterHistoryPut;
  int64_t nCounterHistory[MICROPROFILE_GRAPH_HISTORY]
                         [MICROPROFILE_MAX_COUNTERS]; // flipped to make
                                                      // swapping cheap, drawing
                                                      // more expensive.
...
inline MicroProfileLogEntry MicroProfileMakeLogIndex(uint64_t nBegin,
                                                     MicroProfileToken nToken,
                                                     int64_t nTick) {
...
inline MicroProfileLogEntry MicroProfileMakeLogPayload(const char *pData,
                                                       uint64_t nSize) {
  MP_ASSERT(nSize < 8);
...
inline int64_t MicroProfileLogTickDifference(MicroProfileLogEntry Start,
                                             MicroProfileLogEntry End) {
...
inline int64_t MicroProfileLogTickMax(MicroProfileLogEntry A,
                                      MicroProfileLogEntry B) {
...
inline int64_t MicroProfileLogTickMin(MicroProfileLogEntry A,
                                      MicroProfileLogEntry B) {
  int64_t Diff = MicroProfileLogTickDifference(A, B);
  if (Diff < 0) {
    return B;
...
inline int64_t MicroProfileLogTickClamp(uint64_t T, uint64_t min,
                                        uint64_t max) {
...
inline MicroProfileToken MicroProfileMakeToken(uint32_t nGroupMask,
                                               uint16_t nGroupIndex,
                                               uint16_t nTimer) {
  return ((uint64_t)nGroupIndex << 48llu) | ((uint64_t)nGroupMask << 16llu) |
         nTimer;
}

template <typename T> T MicroProfileMin(T a, T b) { return a < b ? a : b; }

template <typename T> T MicroProfileMax(T a, T b) { return a > b ? a : b; }
template <typename T> T MicroProfileClamp(T a, T min_, T max_) {
...
uint16_t MicroProfileGetGroupIndex(MicroProfileToken t) {
  return (uint16_t)MicroProfileGet()
      ->TimerToGroup[MicroProfileGetTimerIndex(t)];
}

uint64_t MicroProfileTick() { return MP_TICK(); }
...
#ifndef _WIN32
typedef pthread_t MicroProfileThread;
void MicroProfileThreadStart(MicroProfileThread *pThread,
                             MicroProfileThreadFunc Func) {
  pthread_attr_t Attr;
...
#include <thread>
typedef std::thread *MicroProfileThread;
inline void MicroProfileThreadStart(MicroProfileThread *pThread,
                                    MicroProfileThreadFunc Func) {
...
static bool g_bUseLock =
    false; /// This is used because windows does not support using mutexes under
           /// dll init(which is where global initialization is handled)

MICROPROFILE_DEFINE(g_MicroProfileFlip, "MicroProfile", "MicroProfileFlip",
                    MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileThreadLoop, "MicroProfile", "ThreadLoop",
                    MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileClear, "MicroProfile", "Clear", MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileAccumulate, "MicroProfile", "Accumulate",
                    MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileContextSwitchSearch, "MicroProfile",
                    "ContextSwitchSearch", MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileGpuSubmit, "MicroProfile",
                    "MicroProfileGpuSubmit", MP_HOTPINK2);
MICROPROFILE_DEFINE(g_MicroProfileSendLoop, "MicroProfile",
                    "MicroProfileSocketSendLoop", MP_GREEN4);
MICROPROFILE_DEFINE_LOCAL_ATOMIC_COUNTER(g_MicroProfileBytesPerFlip,
                                         "microprofile/bytesperflip");

void MicroProfileHashTableInit(MicroProfileHashTable *pTable,
                               uint32_t nInitialSize,
                               MicroProfileHashCompareFunction CompareFunc,
                               MicroProfileHashFunction HashFunc);
...
bool MicroProfileHashTableSet(MicroProfileHashTable *pTable, uint64_t Key,
                              uintptr_t Value, uint64_t H, bool bAllowGrow);
bool MicroProfileHashTableGet(MicroProfileHashTable *pTable, uint64_t Key,
                              uintptr_t *pValue);
bool MicroProfileHashTableRemove(MicroProfileHashTable *pTable, uint64_t Key);

bool MicroProfileHashTableSetString(MicroProfileHashTable *pTable,
                                    const char *pKey, const char *pValue);
bool MicroProfileHashTableGetString(MicroProfileHashTable *pTable,
                                    const char *pKey, const char **pValue);
bool MicroProfileHashTableRemoveString(MicroProfileHashTable *pTable,
                                       const char *pKey);
enum {
...
void MicroProfileCounterTokenInitName(MicroProfileToken nToken,
                                      const char *pName);
void MicroProfileCounterConfigInternal(MicroProfileToken, uint32_t eFormat,
                                       int64_t nLimit, uint32_t nFlags);

inline std::recursive_mutex &MicroProfileMutex() {
  static std::recursive_mutex Mutex;
  return Mutex;
}
std::recursive_mutex &MicroProfileGetMutex() { return MicroProfileMutex(); }

inline std::recursive_mutex &MicroProfileTimelineMutex() {
  static std::recursive_mutex Mutex;
  return Mutex;
}
MICROPROFILE_API MicroProfile *MicroProfileGet() { return &g_MicroProfile; }
...
void MicroProfileInit() {
  ...
  if (bOnce) {
    ...
    S.CounterToken_StringBlock =
        MicroProfileCounterTokenInit(S.CounterToken_MicroProfile);
    S.CounterToken_StringBlock_Count =
        MicroProfileCounterTokenInit(S.CounterToken_StringBlock);
    S.CounterToken_StringBlock_Waste =
        MicroProfileCounterTokenInit(S.CounterToken_StringBlock);
    S.CounterToken_StringBlock_Strings =
        MicroProfileCounterTokenInit(S.CounterToken_StringBlock);
    S.CounterToken_StringBlock_Memory =
        MicroProfileCounterTokenInit(S.CounterToken_StringBlock);

    S.CounterToken_Alloc =
        MicroProfileCounterTokenInit(S.CounterToken_MicroProfile);
    S.CounterToken_Alloc_Memory =
        MicroProfileCounterTokenInit(S.CounterToken_Alloc);
    S.CounterToken_Alloc_Count =
        MicroProfileCounterTokenInit(S.CounterToken_Alloc);

    MicroProfileCounterTokenInitName(S.CounterToken_MicroProfile,
                                     "microprofile");
    ...
    MicroProfileCounterTokenInitName(S.CounterToken_StringBlock_Strings,
                                     "strings");
    MicroProfileCounterTokenInitName(S.CounterToken_StringBlock_Memory,
                                     "memory");
    ...
    for (int i = 0; i < MICROPROFILE_MAX_CATEGORIES; ++i) {
      ...
      memset(S.CategoryInfo[i].nGroupMask, 0,
             sizeof(S.CategoryInfo[i].nGroupMask));
    }
    ...
    S.nWebServerPort =
        MICROPROFILE_WEBSERVER_PORT; // Use defined value as default port
    ...
    S.nTokenNegativeCpu = MicroProfileGetToken(
        "NEGATIVE_CPU", "NEGATIVE_CPU", MP_PURPLE, MicroProfileTokenTypeCpu);
    S.nTokenNegativeGpu = MicroProfileGetToken(
        "NEGATIVE_GPU", "NEGATIVE_GPU", MP_PURPLE, MicroProfileTokenTypeGpu);
  ...
  MicroProfileCounterConfigInternal(S.CounterToken_Alloc_Memory,
                                    MICROPROFILE_COUNTER_FORMAT_BYTES, 0,
                                    MICROPROFILE_COUNTER_FLAG_DETAILED);
  MICROPROFILE_COUNTER_CONFIG("MicroProfile/ThreadLog/Memory",
                              MICROPROFILE_COUNTER_FORMAT_BYTES, 0,
                              MICROPROFILE_COUNTER_FLAG_DETAILED);
...
inline MicroProfileThreadLog *MicroProfileGetThreadLog() {
  ...
  return (MicroProfileThreadLog *)pthread_getspecific(
      g_MicroProfileThreadLogKey);
}
...
struct MicroProfileScopeLock {
  ...
  MicroProfileScopeLock(std::recursive_mutex &m)
      : bUseLock(g_bUseLock), nUnlock(0), m(m) {
    if (bUseLock)
      m.lock();
  }
...
MicroProfileThreadLog *MicroProfileCreateThreadLog(const char *pName) {
  ...
  if (S.nFreeListHead != -1) {
    ...
    MICROPROFILE_COUNTER_ADD("MicroProfile/ThreadLog/Memory",
                             sizeof(MicroProfileThreadLog));
  ...
  if (pName) {
    ...
    len = snprintf(&pLog->ThreadName[0], sizeof(pLog->ThreadName) - 1,
                   "TID:[%" PRId64 "]", (int64_t)MP_GETCURRENTTHREADID());
  }
...
int MicroProfileInitGpuQueue(const char *pQueueName) {
  for (uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i) {
    ...
    if (pLog && 0 == MP_STRCASECMP(pQueueName, pLog->ThreadName)) {
      ...
      MP_ASSERT(0); // call MicroProfileInitGpuQueue only once per CommandQueue.
                    // name must not clash with threadname
    }
  }
...
MICROPROFILE_API int MicroProfileGetGlobalGpuQueue() { return S.GpuQueue; }
...
void MicroProfileInitThreadLog() { MicroProfileOnThreadCreate(nullptr); }
...
void MicroProfileRegisterGroup(const char *pGroup, const char *pCategory,
                               uint32_t nColor) {
  MicroProfileScopeLock L(MicroProfileMutex());
  ...
  uint16_t nGroup = MicroProfileGetGroup(
      pGroup, 0 != MP_STRCASECMP(pGroup, "gpu") ? MicroProfileTokenTypeCpu
...
MicroProfileToken MicroProfileGetToken(const char *pGroup, const char *pName,
                                       uint32_t nColor,
                                       MicroProfileTokenType Type) {
  MicroProfileInit();
  MicroProfileScopeLock L(MicroProfileMutex());
  ...
  S.GroupInfo[nGroupIndex].nMaxTimerNameLen = MicroProfileMax(
      S.GroupInfo[nGroupIndex].nMaxTimerNameLen, (uint32_t)strlen(pName));
  MP_ASSERT(S.GroupInfo[nGroupIndex].Type ==
            Type); // dont mix cpu & gpu timers in the same group
  S.nMaxGroupSize =
      MicroProfileMax(S.nMaxGroupSize, S.GroupInfo[nGroupIndex].nNumTimers);
  ...
  snprintf(&S.TimerInfo[nTimerIndex].pNameExt[0],
           sizeof(S.TimerInfo[nTimerIndex].pNameExt) - 1, "%s %s",
           S.GroupInfo[nGroupIndex].pName, pName);
...
void MicroProfileGetTokenC(MicroProfileToken *pToken, const char *pGroup,
                           const char *pName, uint32_t nColor,
                           MicroProfileTokenType Type) {
...
const char *MicroProfileNextName(const char *pName, char *pNameOut,
                                 uint32_t *nSubNameLen) {
...
const char *MicroProfileCounterFullName(int nCounter) {
  ...
  while (nIndex >= 0 && nOffset < (int)sizeof(Buffer) - 2) {
    uint32_t nLen =
        S.CounterInfo[nNodes[nIndex]].nNameLen + nOffset; // < sizeof(Buffer)-1
...
// S.CounterTokenStringBlock =
// MicroProfileCounterTokenInit(S.CounterTokenMicroProfile);
MicroProfileToken MicroProfileCounterTokenInit(int nParent) {
...
MicroProfileToken MicroProfileGetCounterTokenByParent(int nParent,
                                                      const char *pName) {
  for (uint32_t i = 0; i < S.nNumCounters; ++i) {
    if (nParent == S.CounterInfo[i].nParent &&
        S.CounterInfo[i].pName == pName) {
      return i;
    }
  }
...
inline void MicroProfileLogPut(MicroProfileLogEntry LE,
                               MicroProfileThreadLog *pLog) {
  MP_ASSERT(
      pLog !=
      0); // this assert is hit if MicroProfileOnCreateThread is not called
...
inline uint64_t MicroProfileLogPutEnter(MicroProfileToken nToken_,
                                        uint64_t nTick,
                                        MicroProfileThreadLog *pLog) {
  MP_ASSERT(
      pLog !=
...
inline void MicroProfileLogPutLeave(MicroProfileToken nToken_, uint64_t nTick,
                                    MicroProfileThreadLog *pLog) {
  MP_ASSERT(
      pLog !=
      0); // this assert is hit if MicroProfileOnCreateThread is not called
...
inline void MicroProfileLogPut(MicroProfileToken nToken_, uint64_t nTick,
                               uint64_t nBegin, MicroProfileThreadLog *pLog) {
  MicroProfileLogPut(MicroProfileMakeLogIndex(nBegin, nToken_, nTick), pLog);
}

inline void MicroProfileLogPutGpu(MicroProfileLogEntry LE,
...
inline void MicroProfileLogPutGpu(MicroProfileToken nToken_, uint64_t nTick,
                                  uint64_t nBegin,
                                  MicroProfileThreadLogGpu *pLog) {
...
uint64_t MicroProfileEnterInternal(MicroProfileToken nToken_) {
  if (MicroProfileGroupTokenActive(nToken_)) {
    ...
    if (MICROPROFILE_PLATFORM_MARKERS_ENABLED) {
      ...
      return MicroProfileLogPutEnter(nToken_, nTick,
                                     MicroProfileGetThreadLog2());
    }
  }
  return MICROPROFILE_INVALID_TICK;
}
...
void MicroProfileTimelineLeave(uint32_t id) {
  ...
  {
    uint32_t nFrameStart =
        S.TimelineTokenFrame[id % MICROPROFILE_TIMELINE_MAX_TOKENS];
    ...
    uint32_t nFrameDistance =
        (nFrameCurrent - nFrameStart) % MICROPROFILE_MAX_FRAME_HISTORY;
    S.TimelineTokenFrame[id % MICROPROFILE_TIMELINE_MAX_TOKENS] =
  ...
  if (nDistance < 2 + 4) {
    ...
    uint64_t LEEnter =
        MicroProfileMakeLogIndex(MP_LOG_LEAVE, ETOKEN_CUSTOM_NAME, MP_TICK());
    uint64_t LEId =
        MicroProfileMakeLogIndex(MP_LOG_EXTRA_DATA, ETOKEN_CUSTOM_ID, id);
...
void MicroProfileTimelineEnterStatic(uint32_t nColor, const char *pStr) {
  uint32_t nToken = MicroProfileTimelineEnterInternal(
      nColor, pStr, (uint32_t)strlen(pStr), true);
  (void)nToken;
}
...
uint32_t MicroProfileTimelineEnterInternal(uint32_t nColor, const char *pStr,
                                           uint32_t nStrLen,
                                           int bIsStaticString) {
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileTimelineMutex());
  MicroProfileThreadLog *pLog = &S.TimelineLog;
  ...
  if (nDistance < nNumMessages + 7) {
    ...
    {
      while (token == 0 ||
             S.TimelineTokenFrame[token % MICROPROFILE_TIMELINE_MAX_TOKENS] !=
                 MICROPROFILE_INVALID_FRAME) {
    ...
    if (bIsStaticString) {
      S.TimelineTokenStaticString[token % MICROPROFILE_TIMELINE_MAX_TOKENS] =
          pStr;
    } else {
      S.TimelineTokenStaticString[token % MICROPROFILE_TIMELINE_MAX_TOKENS] =
          nullptr;
    ...
    uint64_t LEEnter =
        MicroProfileMakeLogIndex(MP_LOG_ENTER, ETOKEN_CUSTOM_NAME, MP_TICK());
    uint64_t LEColor = MicroProfileMakeLogIndex(MP_LOG_EXTRA_DATA,
                                                ETOKEN_CUSTOM_COLOR, nColor);
    uint64_t LEId =
        MicroProfileMakeLogIndex(MP_LOG_EXTRA_DATA, ETOKEN_CUSTOM_ID, token);
...
uint32_t MicroProfileTimelineEnter(uint32_t nColor, const char *pStr) {
  return MicroProfileTimelineEnterInternal(nColor, pStr, (uint32_t)strlen(pStr),
                                           false);
}
...
uint32_t MicroProfileTimelineEnterf(uint32_t nColor, const char *pStr, ...) {
  ...
  buffer[size] = '\0';
  return MicroProfileTimelineEnterInternal(nColor, buffer, (uint32_t)size,
                                           false);
}
...
void MicroProfileLocalCounterAddAtomic(MicroProfileToken nToken,
                                       int64_t nCount) {
  std::atomic<int64_t> *pCounter = &S.CounterInfo[nToken].ExternalAtomic;
  pCounter->fetch_add(nCount);
}
int64_t MicroProfileLocalCounterSetAtomic(MicroProfileToken nToken,
                                          int64_t nCount) {
...
void MicroProfileCounterConfigInternal(MicroProfileToken nToken,
                                       uint32_t eFormat, int64_t nLimit,
                                       uint32_t nFlags) {
  S.CounterInfo[nToken].eFormat = (MicroProfileCounterFormat)eFormat;
  S.CounterInfo[nToken].nLimit = nLimit;
  S.CounterInfo[nToken].nFlags |=
      (nFlags & ~MICROPROFILE_COUNTER_FLAG_INTERNAL_MASK);
}
...
void MicroProfileCounterSetPtr(const char *pCounterName, void *pSource,
                               uint32_t nSize) {
...
void MicroProfileEnter(MicroProfileToken nToken) {
  ...
  MP_ASSERT(
      pLog->nStackScope <
      MICROPROFILE_STACK_MAX); // if youre hitting this assert you probably have
...
void MicroProfileLeave() {
  MicroProfileThreadLog *pLog = MicroProfileGetThreadLog2();
  MP_ASSERT(pLog->nStackScope > 0); // if youre hitting this assert you probably
                                    // have mismatched _ENTER/_LEAVE markers
  uint32_t nStackPos = --pLog->nStackScope;
  if (nStackPos < MICROPROFILE_STACK_MAX) {
    MicroProfileScopeStateC *pScopeState = &pLog->ScopeState[nStackPos];
...
void MicroProfileEnterGpu(MicroProfileToken nToken,
                          MicroProfileThreadLogGpu *pLog) {
  // MP_ASSERT(pLog->nStackScope < MICROPROFILE_STACK_MAX); // if youre hitting
  // this assert you probably have mismatched _ENTER/_LEAVE markers
  uint32_t nStackPos = pLog->nStackScope++;
  if (nStackPos < MICROPROFILE_STACK_MAX) {
    MicroProfileScopeStateC *pScopeState = &pLog->ScopeState[nStackPos];
    pScopeState->Token = nToken;
...
void MicroProfileEnterNegative() { MicroProfileEnter(S.nTokenNegativeCpu); }
...
void MicroProfileGpuBegin(void *pContext, MicroProfileThreadLogGpu *pLog) {
  MP_ASSERT(pLog->pContext ==
            (void *)-1); // dont call begin without calling end
...
uint64_t MicroProfileGpuEnterInternal(MicroProfileThreadLogGpu *pGpuLog,
                                      MicroProfileToken nToken_) {
  if (MicroProfileGroupTokenActive(nToken_)) {
    if (!MicroProfileGetThreadLog()) {
      MicroProfileInitThreadLog();
    }

    MP_ASSERT(pGpuLog->pContext !=
              (void *)-1); // must be called between GpuBegin/GpuEnd
    uint64_t nTimer = MicroProfileGpuInsertTimeStamp(pGpuLog->pContext);
    MicroProfileLogPutGpu(nToken_, nTimer, MP_LOG_ENTER, pGpuLog);
    MicroProfileThreadLog *pLog = MicroProfileGetThreadLog();
    MicroProfileLogPutGpu(ETOKEN_GPU_CPU_TIMESTAMP, MP_TICK(),
                          MP_LOG_EXTRA_DATA, pGpuLog);
    MicroProfileLogPutGpu(ETOKEN_GPU_CPU_SOURCE_THREAD, pLog->nLogIndex,
                          MP_LOG_EXTRA_DATA, pGpuLog);
...
void MicroProfileGpuLeaveInternal(MicroProfileThreadLogGpu *pGpuLog,
                                  MicroProfileToken nToken_,
                                  uint64_t nTickStart) {
  if (nTickStart) {
    if (!MicroProfileGetThreadLog()) {
...
void MicroProfileContextSwitchPut(MicroProfileContextSwitch *pContextSwitch) {
  if (0 == S.nPauseTicks || pContextSwitch->nTicks <= S.nPauseTicks) {
    ...
    S.nContextSwitchPut =
        (S.nContextSwitchPut + 1) % MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE;
...
void MicroProfileToggleFrozen() { S.nFrozen = !S.nFrozen; }

int MicroProfileIsFrozen() { return S.nFrozen != 0 ? 1 : 0; }
int MicroProfileEnabled() { return MicroProfileAnyGroupActive(); }
void *MicroProfileAllocInternal(size_t nSize, size_t nAlign) {
...
void MicroProfileFlip_CB(void *pContext, MicroProfileOnFreeze FreezeCB) {
  ...
  if (S.nWebServerDataSent == (uint64_t)-1) {
    ...
    if (!S.WebSocketThreadRunning) {
      ...
      MicroProfileThreadStart(&S.WebSocketSendThread,
                              MicroProfileSocketSenderThread);
    }
  }
  ...
  uint32_t nAggregateClear = S.nAggregateClear || S.nAutoClearFrames,
           nAggregateFlip = 0;
  ...
  if (nRunning) {
    ...
    MP_ASSERT((S.nFramePutIndex % MICROPROFILE_MAX_FRAME_HISTORY) ==
              S.nFramePut);
    S.nFrameCurrent = (S.nFramePut + MICROPROFILE_MAX_FRAME_HISTORY -
                       MICROPROFILE_GPU_FRAME_DELAY - 1) %
    ...
    pFramePut->nFrameStartGpu = nGpuTimeStamp;
    {
      const float fDumpTimeThreshold =
          1000.f * 60 * 60 * 24 * 365.f; // if time above this, then we're
                                         // handling uninitialized counters
    ...
    const bool bGpuFrameInvalid =
        nTickEndFrameGpu_ == MICROPROFILE_INVALID_TICK ||
        nTickStartFrameGpu_ == MICROPROFILE_INVALID_TICK;
    const uint64_t nTickEndFrameGpu = bGpuFrameInvalid ? 1 : nTickEndFrameGpu_;
    const uint64_t nTickStartFrameGpu =
        bGpuFrameInvalid ? 2 : nTickStartFrameGpu_;
...
#define MP_ASSERT_LE_WRAP(l, g) MP_ASSERT(uint64_t(g - l) < 0x8000000000000000)
      ...
      if (!pLog) {
        ...
        if (!pLog->nGpu) {
          ...
          while (nStart != nPut) {
            ...
            int64_t nDifference =
                MicroProfileLogTickDifference(LE, nTickEndFrame);
            if (nDifference > 0) {
    ...
    {
      pFramePut->nLogStartTimeline =
          S.TimelineLog.nPut.load(std::memory_order_acquire);
...

    {
      ...
      }
      {
        ...
        for (uint32_t idx_thread = 0; idx_thread < MICROPROFILE_MAX_THREADS;
             ++idx_thread) {
          ...
          float fToMs = bGpu ? MicroProfileTickToMsMultiplierGpu()
          ...
          MicroProfile::GroupTime *pFrameGroupThread =
              &S.FrameGroupThread[idx_thread][0];

          const uint32_t nTimerNegative =
              bGpu ? S.nTimerNegativeGpuIndex : S.nTimerNegativeCpuIndex;
          ...
          if (nPut != nGet) {
            ...
            memset(pFrameGroupThread, 0,
                   sizeof(S.FrameGroupThread[idx_thread]));
          }
          ...
          for (uint32_t j = 0; j < 2; ++j) {
            ...
            for (uint32_t k = nStart; k < nEnd; ++k) {
              ...
              if (MP_LOG_ENTER == nType) {
                ...
                {
                  nStackPos--;
                  uint64_t nTickStart = MicroProfileLogTickClamp(
                      pStackLog[nStackPos], nTickStartLog, nTickEndLog);
                  uint64_t nNegative = nNegativeStack[nStackPos];
                  nNegativeStack[nStackPos] = 0;
                  uint64_t nClamped =
                      MicroProfileLogTickClamp(LE, nTickStartLog, nTickEndLog);
          ...
          for (int32_t i = nStackPos - 1; i >= 0; --i) {
            ...
            uint64_t nTickStart =
                MicroProfileLogTickClamp(LE, nTickStartLog, nTickEndLog);
            uint64_t nTicks =
                MicroProfileLogTickDifference(nTickStart, nTickEndLog);
          ...
          for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j) {
            ...
            if ((S.FrameGroupThreadValid[idx_thread / 32] &
                 (1 << (idx_thread % 32))) != 0) {
              pFrameGroup[j].nTicks += pFrameGroupThread[j].nTicks;
              pFrameGroup[j].nTicksExclusive +=
                  pFrameGroupThread[j].nTicksExclusive;
          ...
          if (pLog->nActive == 2 &&
              pLog->nIdleFrames > MICROPROFILE_THREAD_LOG_FRAMES_REUSE) {
      ...
      }
      {
        ...
        for (uint32_t i = 0; i < S.nTotalTimers; ++i) {
          ...
          S.AccumMaxTimers[i] =
              MicroProfileMax(S.AccumMaxTimers[i], S.Frame[i].nTicks);
          S.AccumMinTimers[i] =
              MicroProfileMin(S.AccumMinTimers[i], S.Frame[i].nTicks);
          S.AccumTimersExclusive[i] += S.FrameExclusive[i];
          S.AccumMaxTimersExclusive[i] = MicroProfileMax(
              S.AccumMaxTimersExclusive[i], S.FrameExclusive[i]);
        }

        for (uint32_t i = 0; i < MICROPROFILE_MAX_GROUPS; ++i) {
          S.AccumGroup[i] += pFrameGroup[i].nTicks;
          S.AccumGroupMax[i] =
              MicroProfileMax(S.AccumGroupMax[i], pFrameGroup[i].nTicks);
        }
      }
      for (uint32_t i = 0; i < MICROPROFILE_MAX_GRAPHS; ++i) {
  ...
  if (nAggregateFlip) {
    memcpy(&S.Aggregate[0], &S.AccumTimers[0],
           sizeof(S.Aggregate[0]) * S.nTotalTimers);
    ...
    memcpy(&S.AggregateGroupMax[0], &S.AccumGroupMax[0],
           sizeof(S.AggregateGroup));
    ...
    if (nAggregateClear) {
      ...
      memset(&S.AccumMaxTimers[0], 0,
             sizeof(S.AccumMaxTimers[0]) * S.nTotalTimers);
      memset(&S.AccumMinTimers[0], 0xFF,
             sizeof(S.AccumMinTimers[0]) * S.nTotalTimers);
      memset(&S.AccumTimersExclusive[0], 0,
             sizeof(S.AggregateExclusive[0]) * S.nTotalTimers);
    ...
    S.nCounterHistoryPut =
        (S.nCounterHistoryPut + 1) % MICROPROFILE_GRAPH_HISTORY;
    for (uint32_t i = 0; i < S.nNumCounters; ++i) {
...
void MicroProfileEnableCategory(const char *pCategory, int bEnabled) {
  ...
  if (nCategoryIndex >= 0) {
    if (bEnabled) {
      for (uint32_t i = 0; i < MICROPROFILE_MAX_GROUP_INTS; ++i) {
        S.nActiveGroupsWanted[i] |=
            S.CategoryInfo[nCategoryIndex].nGroupMask[i];
      }
    } else {
      for (uint32_t i = 0; i < MICROPROFILE_MAX_GROUP_INTS; ++i) {
        S.nActiveGroupsWanted[i] &=
...
int MicroProfileGetForceMetaCounters() { return 0; }
...
int MicroProfileGetAggregateFrames() { return S.nAggregateFlip; }

int MicroProfileGetCurrentAggregateFrames() {
  return int(S.nAggregateFlip ? S.nAggregateFlip : S.nAggregateFlipCount);
}

void MicroProfileForceEnableGroup(const char *pGroup,
                                  MicroProfileTokenType Type) {
  MicroProfileInit();
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileMutex());
...
void MicroProfileForceDisableGroup(const char *pGroup,
                                   MicroProfileTokenType Type) {
  MicroProfileInit();
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileMutex());
  uint16_t nGroup = MicroProfileGetGroup(pGroup, Type);
...
void MicroProfileCalcAllTimers(float *pTimers, float *pAverage, float *pMax,
                               float *pMin, float *pCallAverage,
                               float *pExclusive, float *pAverageExclusive,
                               float *pMaxExclusive, float *pTotal,
                               uint32_t nSize) {
  for (uint32_t i = 0; i < S.nTotalTimers && i < nSize; ++i) {
    ...
    const float fToMs = MicroProfileTickToMsMultiplier(
        S.GroupInfo[nGroupId].Type == MicroProfileTokenTypeGpu
            ? MicroProfileTicksPerSecondGpu()
            : MicroProfileTicksPerSecondCpu());
    uint32_t nTimer = i;
    uint32_t nIdx = i * 2;
    ...
    float fMinMs =
        fToMs *
        (S.AggregateMin[nTimer] != uint64_t(-1) ? S.AggregateMin[nTimer] : 0);
...
void MicroProfileContextSwitchSearch(uint32_t *pContextSwitchStart,
                                     uint32_t *pContextSwitchEnd,
                                     uint64_t nBaseTicksCpu,
  ...
  for (uint32_t i = 0; i < MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE; ++i) {
    uint32_t nIndex = (nContextSwitchPut +
                       MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE - (i + 1)) %
                      MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE;
  ...
  *pContextSwitchEnd = nContextSwitchEnd;
...
#if MICROPROFILE_CONTEXT_SWITCH_SEARCH_DEBUG
  {
    ...
    for (uint32_t i = 0; i < MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE; ++i) {
      ...
      if (fMs < 0 || fMs > 50) {
        // dump range here
        uprintf("range [%5" PRId64 ":%5" PRId64
                "] :: [%6.2f:%6.2f]                 [%p :: %p] .. ref %p\n",
                nBegin, nEnd, fToMs * (nMin - nSearchBegin),
                fToMs * (nMax - nSearchBegin), (void *)nMin, (void *)nMax,
                (void *)nSearchBegin
...
int MicroProfileFormatCounter(int eFormat, int64_t nCounter, char *pOut,
                              uint32_t nBufferSize) {
  if (!nCounter) {
    pOut[0] = '0';
    pOut[1] = '\0';
  ...
  case MICROPROFILE_COUNTER_FORMAT_BYTES: {
    ...
    if (nShift) {
      ...
      nLen = snprintf(pOut, nBufferSize - 1, "%c%" PRId64 "%s",
                      nNegative ? '-' : ' ', nCounter, pExt[nShift]);
    }
  } break;
  }
  pBase[nLen] = '\0';
...
typedef void (*MicroProfileWriteCallback)(void *Handle, size_t size,
                                          const char *pData);

uint32_t MicroProfileWebServerPort() { return S.nWebServerPort; }
...
void MicroProfileSetWebServerPort(uint32_t nPort) {
  if (S.nWebServerPort != nPort) {
    ...
    S.nWebServerDataSent =
        (uint64_t)-1; // Will cause the web server and its thread to be
                      // restarted next time MicroProfileFlip() is called.
  }
}

void MicroProfileDumpFileImmediately(const char *pHtml, const char *pCsv,
                                     void *pGpuContext) {
...
void MicroProfileDumpFile(const char *pHtml, const char *pCsv, float fCpuSpike,
                          float fGpuSpike) {
...
void MicroProfilePrintf(MicroProfileWriteCallback CB, void *Handle,
                        const char *pFmt, ...) {
  va_list args;
  va_start(args, pFmt);
  ...
  int size = stbsp_vsprintfcb(MicroProfilePrintfCallback, (void *)&A, Buffer,
                              pFmt, args);
  (void)size;
  va_end(args);
}
...
void MicroProfileDumpCsv(MicroProfileWriteCallback CB, void *Handle) {
  ...
  MicroProfileCalcAllTimers(pTimers, pAverage, pMax, pMin, pCallAverage,
  ...
  for (uint32_t i = 0; i < S.nTotalTimers; ++i) {
    uint32_t nIdx = i * 2;
    printf("\"%s\",\"%s\",%f,%f,%f\n", S.TimerInfo[i].pName,
           S.GroupInfo[S.TimerInfo[i].nGroupIndex].pName, pAverage[nIdx],
  ...
  for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j) {
    ...
    float fToMs =
        S.GroupInfo[j].Type == MicroProfileTokenTypeGpu ? fToMsGPU : fToMsCPU;
    if (pGroupName[0] != '\0') {
      printf("\"%s\",%.3f,%.3f,%.3f\n", pGroupName,
             fToMs * S.AggregateGroup[j] / nAggregateFrames,
             fToMs * S.AggregateGroup[j] / nAggregateFrames,
             fToMs * S.AggregateGroup[j]);
    }
  }
  ...
  for (uint32_t i = nCount; i > 0; i--) {
    uint32_t nFrame = (nStart + MICROPROFILE_MAX_FRAME_HISTORY - i) %
                      MICROPROFILE_MAX_FRAME_HISTORY;
    uint32_t nFrameNext = (nStart + MICROPROFILE_MAX_FRAME_HISTORY - i + 1) %
                          MICROPROFILE_MAX_FRAME_HISTORY;
    uint64_t nTicks =
        S.Frames[nFrameNext].nFrameStartCpu - S.Frames[nFrame].nFrameStartCpu;
  ...
  for (uint32_t i = nCount; i > 0; i--) {
    ...
    uint32_t nFrameNext = (nStart + MICROPROFILE_MAX_FRAME_HISTORY - i + 1) %
                          MICROPROFILE_MAX_FRAME_HISTORY;
    uint64_t nTicks =
        S.Frames[nFrameNext].nFrameStartGpu - S.Frames[nFrame].nFrameStartGpu;
...
void MicroProfileDumpHtmlLive(MicroProfileWriteCallback CB, void *Handle) {
  for (size_t i = 0; i < g_MicroProfileHtmlLive_begin_count; ++i) {
    CB(Handle, g_MicroProfileHtmlLive_begin_sizes[i] - 1,
       g_MicroProfileHtmlLive_begin[i]);
  }
  for (size_t i = 0; i < g_MicroProfileHtmlLive_end_count; ++i) {
    CB(Handle, g_MicroProfileHtmlLive_end_sizes[i] - 1,
       g_MicroProfileHtmlLive_end[i]);
  }
}
void MicroProfileDumpHtml(MicroProfileWriteCallback CB, void *Handle,
                          uint64_t nMaxFrames, const char *pHost,
                          uint64_t nStartFrameId = (uint64_t)-1) {
  ...
  int c = 0;
  while (0 == S.nPauseSignal && S.bContextSwitchRunning &&
         !S.bContextSwitchStop) {
    MicroProfileSleep(100);
    if (++c % 100 == 99) {
  ...
  for (size_t i = 0; i < g_MicroProfileHtml_begin_count; ++i) {
    CB(Handle, g_MicroProfileHtml_begin_sizes[i] - 1,
       g_MicroProfileHtml_begin[i]);
  }
  // dump info
  ...
  MicroProfilePrintf(CB, Handle,
                     "S.AggregateInfo = {'Frames':%d, 'Time':%f};\n",
  ...
  for (uint32_t i = 0; i < S.nGroupCount; ++i) {
    ...
    float fToMs =
        S.GroupInfo[i].Type == MicroProfileTokenTypeCpu ? fToMsCPU : fToMsGPU;
    ...
    MicroProfilePrintf(CB, Handle,
                       "S.GroupInfo[%d] = MakeGroup(%d, \"%s\", %d, %d, %d, "
                       "%f, %f, %f, '%s');\n",
                       S.GroupInfo[i].nGroupIndex, S.GroupInfo[i].nGroupIndex,
                       S.GroupInfo[i].pName, S.GroupInfo[i].nCategory,
  ...
  for (uint32_t i = 0; i < S.nTotalTimers; ++i) {
    ...
    MicroProfilePrintf(
        CB, Handle,
        "S.TimerInfo[%d] = MakeTimer(%d, \"%s\", %d, "
        "'#%02x%02x%02x','#%02x%02x%02x', %f, %f, %f, %f, %f, %f, %d, %f, "
        "S.Meta%d, S.MetaAvg%d, S.MetaMax%d);\n",
        S.TimerInfo[i].nTimerIndex, S.TimerInfo[i].nTimerIndex,
        S.TimerInfo[i].pName, S.TimerInfo[i].nGroupIndex,
        ...
        MICROPROFILE_UNPACK_BLUE(nColorDark) & 0xff, pAverage[nIdx], pMax[nIdx],
        pMin[nIdx], pAverageExclusive[nIdx], pMaxExclusive[nIdx],
        pCallAverage[nIdx], S.Aggregate[i].nCount, pTotal[nIdx], i, i, i);
  }
  ...
  MicroProfilePrintf(CB, Handle, "\nS.ISGPU = [");
  for (uint32_t i = 0; i < S.nNumLogs; ++i) {
    MicroProfilePrintf(CB, Handle, "%d,",
                       (S.Pool[i] && S.Pool[i]->nGpu) ? 1 : 0);
  }
  MicroProfilePrintf(CB, Handle, "];\n\n");
    ...
    if (S.Pool[i]) {
      ...
      for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j) {
        MicroProfilePrintf(CB, Handle, "%f,",
                           S.Pool[i]->nAggregateGroupTicks[j] /
                               nAggregateFrames * fToMs);
      }
      MicroProfilePrintf(CB, Handle, "];\n");
    }
  }
  ...
  for (uint32_t i = 0; i < S.nNumLogs; ++i) {
    if (S.Pool[i]) {
      ...
      for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j) {
        MicroProfilePrintf(CB, Handle, "%f,",
                           S.Pool[i]->nAggregateGroupTicks[j] * fToMs);
      }
      MicroProfilePrintf(CB, Handle, "];\n");
    }
  }
  ...
  for (int i = 0; i < (int)S.nNumCounters; ++i) {
    if (0 != (S.CounterInfo[i].nFlags & MICROPROFILE_COUNTER_FLAG_DETAILED)) {
      ...
      for (uint32_t j = 0; j < MICROPROFILE_GRAPH_HISTORY; ++j) {
        ...
        int64_t nValue = MicroProfileClamp(S.nCounterHistory[nHistoryIndex][i],
                                           nCounterMin, nCounterMax);
      ...
      MicroProfilePrintf(
          CB, Handle,
          "S.CounterHistory%d = MakeCounterHistory(%d, "
          "S.CounterHistoryArray%d, S.CounterHistoryArrayPrc%d)\n",
          i, i, i, i);
    } else {
  ...
  for (int i = 0; i < (int)S.nNumCounters; ++i) {
    ...
    MicroProfileFormatCounter(S.CounterInfo[i].eFormat, nCounter, Formatted,
                              sizeof(Formatted) - 1);
    MicroProfileFormatCounter(S.CounterInfo[i].eFormat, S.CounterInfo[i].nLimit,
                              FormattedLimit, sizeof(FormattedLimit) - 1);
    MicroProfilePrintf(
        CB, Handle,
        "MakeCounter(%d, %d, %d, %d, %d, '%s', %lld, %lld, %lld, '%s', %lld, "
        "'%s', %f, %f, %d, S.CounterHistory%d),",
        i, S.CounterInfo[i].nParent, S.CounterInfo[i].nSibling,
        S.CounterInfo[i].nFirstChild, S.CounterInfo[i].nLevel,
        S.CounterInfo[i].pName, nCounter, S.nCounterMin[i], S.nCounterMax[i],
        Formatted, nLimit, FormattedLimit, fCounterPrc, fBoxPrc,
  ...
  if (nStartFrameId != (uint64_t)-1) {
    ...
    if (nFirstFrame != (uint32_t)-1) {
      ...
      uint32_t nDistance =
          (MICROPROFILE_MAX_FRAME_HISTORY + nFirstFrame - nLastFrame) %
          MICROPROFILE_MAX_FRAME_HISTORY;
  ...
  if (nNumFrames == 0) {
    nNumFrames =
        (MICROPROFILE_MAX_FRAME_HISTORY - MICROPROFILE_GPU_FRAME_DELAY -
         3); // leave a few to not overwrite
    nNumFrames = MicroProfileMin(nNumFrames, (uint32_t)nMaxFrames);
    nFirstFrame =
        (S.nFrameCurrent + MICROPROFILE_MAX_FRAME_HISTORY - nNumFrames) %
        MICROPROFILE_MAX_FRAME_HISTORY;
  }

  uint32_t nLastFrame =
      (nFirstFrame + nNumFrames) % MICROPROFILE_MAX_FRAME_HISTORY;
  ...
  if (MicroProfileGetGpuTickReference(&nTickReferenceCpu, &nTickReferenceGpu)) {
    nTickStartGpu = (nTickStart - nTickReferenceCpu) * nTicksPerSecondGpu /
                        nTicksPerSecondCpu +
                    nTickReferenceGpu;
    nTickReference = 1;
  }
  ...
  uint32_t *nTimerCounter =
      (uint32_t *)alloca(sizeof(uint32_t) * S.nTotalTimers);
...

  {
    ...
    uint32_t nFrameIndexLast =
        (nFirstFrame + nNumFrames) % MICROPROFILE_MAX_FRAME_HISTORY;
    {
      // find the frame that has an active marker the furtest distance from the
      // selected range
      int nDelta = 0;
      int nOffset = 0;
      ...
      nFrameIndexFirst =
          (nFirstFrame - nDelta) % MICROPROFILE_MAX_FRAME_HISTORY;
    }
...
#define pp(...) MicroProfilePrintf(CB, Handle, __VA_ARGS__)
  ...
  for (uint32_t i = 0; i < nNumFrames; ++i) {
    ...
    uint32_t nFrameIndexNext =
        (nFrameIndex + 1) % MICROPROFILE_MAX_FRAME_HISTORY;
    ...
    for (uint32_t j = 0; j < S.nNumLogs; ++j) {
        ...
        for (uint32_t k = nLogStart; k != nLogEnd;
          ...
          if (nLogType == MP_LOG_EXTRA_DATA) {
            ...
            if (nTimerIndex == ETOKEN_GPU_CPU_TIMESTAMP) {
              ...
              fTime = MicroProfileLogTickDifference(nStartTick, pLog->Log[k]) *
                      fToMs;
            } else {
              fTime = 0.f;
            }
      ...
      if (nLogStart != nLogEnd) {
        ...
        MicroProfilePrintf(CB, Handle, "%d",
                           MicroProfileLogGetType(pLog->Log[k]));
    ...
    float fToMs = MicroProfileTickToMsMultiplier(nTicksPerSecondCpu);
    float fFrameMs =
        MicroProfileLogTickDifference(nTickStart, nFrameStart) * fToMs;
    float fFrameEndMs =
        MicroProfileLogTickDifference(nTickStart, nFrameEnd) * fToMs;
    float fFrameGpuMs = 0;
    ...
    MicroProfilePrintf(CB, Handle,
                       "S.Frames[%d] = MakeFrame(%d, %f, %f, %f, %f, S.ts%d, "
                       "S.tt%d, S.ti%d);\n",
                       i, 0, fFrameMs, fFrameEndMs, fFrameGpuMs, fFrameGpuEndMs,
                       i, i, i);
  }
  ...
  for (uint32_t j = nContextSwitchStart; j != nContextSwitchEnd;
    ...
    int nCpu = CS.nCpu;
    MicroProfilePrintf(CB, Handle, "%d,%d,%d,", CS.nThreadIn, CS.nThreadOut,
                       nCpu);
  }
  MicroProfilePrintf(CB, Handle, "];\n");
  ...
  for (uint32_t j = nContextSwitchStart; j != nContextSwitchEnd;
    ...
    MicroProfileContextSwitch CS = S.ContextSwitch[j];
    float fTime =
        MicroProfileLogTickDifference(nTickStart, CS.nTicks) * fToMsCpu;
  ...
  for (uint32_t i = 0; i < nNumThreads; ++i) {
    const char *p1 =
        pThreadInfo[i].pThreadModule ? pThreadInfo[i].pThreadModule : "?";
    const char *p2 =
        pThreadInfo[i].pProcessModule ? pThreadInfo[i].pProcessModule : "?";

    MicroProfilePrintf(CB, Handle,
                       "%" PRId64 ":{\'tid\':%" PRId64 ",\'pid\':%" PRId64
                       ",\'t\':\'%s\',\'p\':\'%s\'},",
  ...
  uint32_t *nGroupCounter =
      (uint32_t *)alloca(sizeof(uint32_t) * S.nGroupCount);
  ...
  uint32_t *nGroupCounterSort =
      (uint32_t *)alloca(sizeof(uint32_t) * S.nGroupCount);
  uint32_t *nTimerCounterSort =
      (uint32_t *)alloca(sizeof(uint32_t) * S.nTotalTimers);
  for (uint32_t i = 0; i < S.nGroupCount; ++i) {
  ...
  if (S.nDumpFileNextFrame & 1) {
    ...
    if (F) {
      MicroProfileDumpHtml(MicroProfileWriteFile, F,
                           MICROPROFILE_WEBSERVER_MAXFRAMES, S.HtmlDumpPath);
      fclose(F);
    }
  }
...
void MicroProfileCompressedSocketFlush(
    MicroProfileCompressedSocketState *pState) {
  mz_stream &Stream = pState->Stream;
  unsigned char *pSendStart = &pState->DeflateOut[0];
  unsigned char *pSendEnd =
      &pState->DeflateOut[MICROPROFILE_COMPRESS_CHUNK - Stream.avail_out];
...
void MicroProfileCompressedSocketFinish(
    MicroProfileCompressedSocketState *pState) {
  mz_stream &Stream = pState->Stream;
...
void MicroProfileCompressedWriteSocket(void *Handle, size_t nSize,
                                       const char *pData) {
  MicroProfileCompressedSocketState *pState =
      (MicroProfileCompressedSocketState *)Handle;
  mz_stream &Stream = pState->Stream;
  const unsigned char *pDeflateInEnd = Stream.next_in + Stream.avail_in;
  const unsigned char *pDeflateInStart = &pState->DeflateIn[0];
  const unsigned char *pDeflateInRealEnd =
      &pState->DeflateIn[MICROPROFILE_COMPRESS_CHUNK];
...
#ifndef MicroProfileSetNonBlocking // fcntl doesnt work on a some unix like
                                   // platforms..
...
void MicroProfileWebServerStart() {
...

  {
    ...
    int on = 1;
#if defined(_WIN32)
    r = setsockopt(S.ListenerSocket, SOL_SOCKET, SO_REUSEADDR,
                   (const char *)&on, sizeof(on));
#else
    r = setsockopt(S.ListenerSocket, SOL_SOCKET, SO_REUSEADDR, (void *)&on,
                   sizeof(on));
#endif
    (void)r;
  }
...
MicroProfileGetCommand
MicroProfileParseGet(const char *pGet, MicroProfileParseGetResult *pResult) {
  if (0 == strlen(pGet)) {
...
static void MicroProfile_SHA1_Transform(uint32_t state[5],
...
void MicroProfile_SHA1_Update(MicroProfile_SHA1_CTX *context,
...
void MicroProfile_SHA1_Final(unsigned char digest[20],
                             MicroProfile_SHA1_CTX *context) {
  uint32_t i, j;
  ...
  MicroProfile_SHA1_Update(context, finalcount,
...
void MicroProfileSocketDumpState() {
  ...
  for (uint32_t i = 1; i < S.nNumWebSockets; i++) {
    ...
    int r = getsockopt(s, SOL_SOCKET, SO_ERROR, (char *)&error_code,
                       &error_code_size);
    MP_ASSERT(r >= 0);
    if (error_code != 0) {
#ifdef _WIN32
...
bool MicroProfileSocketSend2(MpSocket Connection, const void *pMessage,
                             int nLen);
void *MicroProfileSocketSenderThread(void *) {
  ...
  while (!S.nMicroProfileShutdown) {
    ...
    if (nSendAmount) {
      ...
      MICROPROFILE_COUNTER_LOCAL_ADD_ATOMIC(g_MicroProfileBytesPerFlip,
                                            nSendAmount);
      if (!MicroProfileSocketSend2(
              S.WebSockets[0], &S.WSBuf.SendBuffer[nSendStart], nSendAmount)) {
        S.nSocketFail = 1;
      } else {
        S.WSBuf.nSendGet.store((nGet + nSendAmount) %
...
void MicroProfileSocketSend(MpSocket Connection, const void *pMessage,
                            int nLen) {
  ...
  while (nLen != 0) {
    ...
    if (nAmount) {
      ...
      nLen -= nAmount;
      S.WSBuf.nSendPut.store((nPut + nAmount) %
                             MICROPROFILE_WEBSOCKET_BUFFER_SIZE);
    } else {
      if (S.nSocketFail) {
        return;
...
void MicroProfileWebSocketIdUnpack(uint32_t nPacked, uint32_t &type,
                                   uint32_t &element) {
...
bool MicroProfileWebSocketSend(MpSocket Connection, const char *pMessage,
                               uint64_t nLen) {
  ...
  MP_ASSERT(pMessage ==
            S.WSBuf.pBuffer); // space for header is preallocated here
  MP_ASSERT(pMessage == S.WSBuf.pBufferAllocation +
                            20); // space for header is preallocated here
...
void MicroProfileToggleCategory(uint32_t nCategory) {
  if (nCategory < S.nCategoryCount) {
    bool bAllSet = true;
    for (uint32_t i = 0; i < MICROPROFILE_MAX_GROUP_INTS; ++i) {
      bAllSet = bAllSet && S.CategoryInfo[nCategory].nGroupMask[i] ==
                               (S.CategoryInfo[nCategory].nGroupMask[i] &
                                S.nActiveGroupsWanted[i]);
    }
    for (uint32_t i = 0; i < MICROPROFILE_MAX_GROUP_INTS; ++i) {
...
void MicroProfileWebSocketCommand(uint32_t nCommand) {
  ...
  case TYPE_SETTING:
    ...
    case SETTING_PLATFORM_MARKERS:
      MicroProfilePlatformMarkersSetEnabled(
...
typedef bool (*MicroProfileOnSettings)(const char *pName, uint32_t nNameLen,
                                       const char *pJson, uint32_t nJson);

#ifndef MICROPROFILE_SETTINGS_FILE_PATH
#define MICROPROFILE_SETTINGS_FILE_PATH ""
#endif
#ifndef MICROPROFILE_SETTINGS_FILE
#define MICROPROFILE_SETTINGS_FILE                                             \
  MICROPROFILE_SETTINGS_FILE_PATH "mppresets.cfg"
#endif
#ifndef MICROPROFILE_SETTINGS_FILE_BUILTIN
#define MICROPROFILE_SETTINGS_FILE_BUILTIN                                     \
  MICROPROFILE_SETTINGS_FILE_PATH "mppresets.builtin.cfg"
#endif
...
void MicroProfileParseSettings(const char *pFileName, T CB) {
  ...
  nFileSize = ftell(F);
  if (nFileSize > (32 << 10)) {
    uprintf("trying to load a >32kb settings file on the stack. this should "
            "never happen!\n");
    MP_BREAK();
  }
  ...
  while (pPos != pEnd) {
    ...
    auto ParseName = [&](char *pPos, char *pEnd, const char **ppName,
                         int *pLen) {
      ...
      *ppName = pPos;
      ...
      while (pPos != pEnd &&
             (isalpha(*pPos) || isdigit(*pPos) || *pPos == '_')) {
        nLen++;
        pPos++;
      }
      *pLen = nLen;
    ...
    auto ParseJson = [&](char *pPos, char *pEnd, const char **pJson,
                         int *pLen) -> char * {
      pPos = SkipWhite(pPos, pEnd);
      if (*pPos != '{' || pPos == pEnd) {
        Failed = 1;
...
bool MicroProfileSavePresets(const char *pSettingsName,
                             const char *pJsonSettings) {
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileGetMutex());
  ...
  MicroProfileParseSettings(MICROPROFILE_SETTINGS_FILE,
                            [&](const char *pName, uint32_t nNameSize,
                                const char *pJson, uint32_t nJsonSize) -> bool {
                              ...
                              if (0 != MP_STRCASECMP(pSettingsName, pName)) {
                                ...
                                bWritten = true;
                                fwrite(pJsonSettings, strlen(pJsonSettings), 1,
                                       F);
                              }
                              fputc('\n', F);
                              return true;
  ...
  MICROPROFILE_MOVE_FILE(MICROPROFILE_SETTINGS_FILE_TEMP,
                         MICROPROFILE_SETTINGS_FILE);
#elif defined(_WIN32)
  MoveFileExA(MICROPROFILE_SETTINGS_FILE_TEMP, MICROPROFILE_SETTINGS_FILE,
...
void MicroProfileWebSocketSendPresets(MpSocket Connection) {
  ...
  MicroProfileParseSettings(MICROPROFILE_SETTINGS_FILE,
                            [](const char *pName, uint32_t nNameLen,
                               const char *pJson, uint32_t nJsonLen) {
  ...
  MicroProfileParseSettings(MICROPROFILE_SETTINGS_FILE_BUILTIN,
                            [&bFirst](const char *pName, uint32_t nNameLen,
                                      const char *pJson, uint32_t nJsonLen) {
...
void MicroProfileLoadPresets(const char *pSettingsName, bool bReadOnlyPreset) {
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileGetMutex());
  const char *pPresetFile = bReadOnlyPreset ? MICROPROFILE_SETTINGS_FILE_BUILTIN
                                            : MICROPROFILE_SETTINGS_FILE;
  MicroProfileParseSettings(pPresetFile, [=](const char *pName, uint32_t l0,
                                             const char *pJson, uint32_t l1) {
...
bool MicroProfileWebSocketReceive(MpSocket Connection) {
  ...
  case 'D': // instrumentation without loading queryable symbols.
  {
    ...
    const char **pModules =
        (const char **)(alloca(sizeof(const char *) * nNumArguments));
    const char **pSymbols =
        (const char **)(alloca(sizeof(const char *) * nNumArguments));
...
void MicroProfileWebSocketHandshake(MpSocket Connection, char *pWebSocketKey) {
  ...
  int nLen = stbsp_snprintf(EncodeBuffer, sizeof(EncodeBuffer) - 1, "%s%s",
                            pWebSocketKey, pGUID);
  // uprintf("encode buffer is '%s' %d, %d\n", EncodeBuffer, nLen,
  // (int)strlen(EncodeBuffer));
  ...
  if (!S.nWSWasConnected) {
    ...
    if (S.pJsonSettings) {
      MicroProfileWSPrintStart(Connection);
      MicroProfileWSPrintf("{\"k\":\"%d\",\"ro\":%d,\"v\":%s}",
                           MSG_CURRENTSETTINGS, S.bJsonSettingsReadOnly ? 1 : 0,
                           S.pJsonSettings);
      MicroProfileWSFlush();
      MicroProfileWSPrintEnd();
    }
...
void MicroProfileWebSocketSendCounters() {
  MICROPROFILE_SCOPEI("MicroProfile", "MicroProfileWebSocketSendCounters",
                      MP_GREEN4);
...
void MicroProfileSymbolSendModuleState() {
  if (S.WSSymbolModulesSent != S.SymbolNumModules ||
      S.nSymbolsDirty.load()) // todo: tag when modulestate is updated.
  {
...
#define FMT "{\"n\":\"%s\",\"a\":\"%llx\",\"s\":\"%lld\", \"p\":%f}"
      MicroProfileWSPrintf(bFirst ? FMT : ("," FMT), pModuleName, nAddrBegin,
                           nNumSymbols, fLoadPrc);
#undef FMT
      bFirst = false;
    }
    MicroProfileWSPrintf("]");
...
void MicroProfileWebSocketSendFrame(MpSocket Connection) {
  if (S.nFrameCurrent != S.WebSocketFrameLast[0] || S.nFrozen) {
    MicroProfileWebSocketSendState(Connection);
    MICROPROFILE_SCOPEI("MicroProfile", "MicroProfileWebSocketSendFrame",
                        MP_GREEN4);
    MicroProfileWSPrintStart(Connection);
    float fTickToMsCpu =
        MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu());
    ...
    uint64_t nFrameTicks =
        pFrameNext->nFrameStartCpu - pFrameCurrent->nFrameStartCpu;
    uint64_t nFrame = pFrameCurrent->nFrameId;
    double fTime = nFrameTicks * fTickToMsCpu;
    MicroProfileWSPrintf(
        "{\"k\":\"%d\",\"v\":{\"t\":%f,\"f\":%lld,\"a\":%d,\"fr\":%d,\"m\":%d",
        MSG_FRAME, fTime, nFrame, MicroProfileGetCurrentAggregateFrames(),
        S.nFrozen, S.nWSViewMode);
#if MICROPROFILE_DYNAMIC_INSTRUMENT
    MicroProfileWSPrintf(
        ",\"s\":{\"n\":%d,\"f\":%d,\"r\":%d,\"l\":%d,\"q\":%d}",
        S.SymbolNumModules, S.SymbolState.nModuleLoadsFinished.load(),
        S.SymbolState.nModuleLoadsRequested.load(),
        S.SymbolState.nSymbolsLoaded.load(), S.pPendingQuery ? 1 : 0);
    MicroProfileSymbolSendModuleState();
#endif

    auto WriteTickArray = [fTickToMsCpu,
                           fTickToMsGpu](MicroProfile::GroupTime *pFrameGroup) {
      MicroProfileWSPrintf("[");
      int f = 0;
      for (uint32_t i = 0; i < MICROPROFILE_MAX_GROUPS; ++i) {
        ...
        if (nTicksExcl) {
          ...
          float fToMs = S.GroupInfo[i].Type == MicroProfileTokenTypeCpu
                            ? fTickToMsCpu
                            : fTickToMsGpu;

          MicroProfileWSPrintf("%c[%f,%f,%f]", f ? ',' : ' ', nTicks * fToMs,
                               nTicksExcl * fToMs, fCount);
          f = 1;
        }
      }
      MicroProfileWSPrintf("]");
      ...
      for (uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i) {
        if (0 != (S.FrameGroupThreadValid[i / 32] & (1 << (i % 32)))) {
          if (!f)
            ...
            MicroProfileWSPrintf(",{");
          MicroProfileThreadLog *pLog = S.Pool[i];
          MicroProfileWSPrintf("\"i\":%d,\"n\":\"%s\",\"g\":", i,
                               pLog->ThreadName);
    ...
    if (S.nFrameCurrent != S.WebSocketFrameLast[0]) {
      ...
      while (-1 != nTimer) {
        ...
        float fTickToMs =
            TI.Type == MicroProfileTokenTypeGpu ? fTickToMsGpu : fTickToMsCpu;
        ...
        nTimer = TI.nWSNext;
        MicroProfileWSPrintf("\"%d\":[%f,%f,%f]%c", id, fTime, fTimeExcl,
                             fCount, nTimer == -1 ? ' ' : ',');
      }
      // uprintf("\n");
      MicroProfileWSPrintf("}");
    ...
    MicroProfileWSPrintStart(Connection);
    MicroProfileWSPrintf("{\"k\":\"%d\",\"v\":{\"fr\":%d,\"m\":%d",
                         MSG_INACTIVE_FRAME, S.nFrozen, S.nWSViewMode);
#if MICROPROFILE_DYNAMIC_INSTRUMENT
    MicroProfileWSPrintf(
        ",\"s\":{\"n\":%d,\"f\":%d,\"r\":%d,\"l\":%d,\"q\":%d}",
        S.SymbolNumModules, S.SymbolState.nModuleLoadsFinished.load(),
  ...
  for (uint32_t i = 0; i < S.nNumWebSockets;) {
    ...
    if (FD_ISSET(s, &Write)) {
      if (S.nJsonSettingsPending) {
        ...
        MicroProfileWSPrintf("{\"k\":\"%d\",\"ro\":%d,\"v\":%s}",
                             MSG_LOADSETTINGS, S.bJsonSettingsReadOnly ? 1 : 0,
                             S.pJsonSettings);
        MicroProfileWSFlush();
        MicroProfileWSPrintEnd();
        S.nJsonSettingsPending = 0;
...
void MicroProfileResizeWSBuf(uint32_t nMinSize = 0) {
  uint32_t nNewSize = MicroProfileMax(
      S.WSBuf.nPut + 2 * (nMinSize + 2 + 20),
      MicroProfileMax(S.WSBuf.nBufferSize * 3 / 2,
                      (uint32_t)MICROPROFILE_WEBSOCKET_BUFFER_SIZE));
  S.WSBuf.pBufferAllocation =
      (char *)MICROPROFILE_REALLOC(S.WSBuf.pBufferAllocation, nNewSize);
...
void MicroProfileWSPrintf(const char *pFmt, ...) {
  ...
  stbsp_vsprintfcb(MicroProfileWSPrintfCallback, 0,
                   S.WSBuf.pBuffer + S.WSBuf.nPut, pFmt, args);
  va_end(args);
}
...
void MicroProfileWebSocketSendEnabledMessage(uint32_t id, int bEnabled) {
  MicroProfileWSPrintf("{\"k\":\"%d\",\"v\":{\"id\":%d,\"e\":%d}}", MSG_ENABLED,
                       id, bEnabled ? 1 : 0);
  MicroProfileWSFlush();
}
void MicroProfileWebSocketSendEnabled(MpSocket C) {
  ...
  for (uint32_t i = 0; i < S.nCategoryCount; ++i) {
    MicroProfileWebSocketSendEnabledMessage(
        MicroProfileWebSocketIdPack(TYPE_CATEGORY, i),
        MicroProfileCategoryEnabled(i));
  }

  for (uint32_t i = 0; i < S.nGroupCount; ++i) {
    MicroProfileWebSocketSendEnabledMessage(
        MicroProfileWebSocketIdPack(TYPE_GROUP, i),
        MicroProfileGroupEnabled(i));
  }
  for (uint32_t i = 0; i < S.nTotalTimers; ++i) {
    MicroProfileWebSocketSendEnabledMessage(
        MicroProfileWebSocketIdPack(TYPE_TIMER, i),
        MicroProfileWebSocketTimerEnabled(i));
  }
  MicroProfileWebSocketSendEnabledMessage(
...
void MicroProfileWebSocketSendEntry(uint32_t id, uint32_t parent,
                                    const char *pName, int nEnabled,
                                    uint32_t nColor) {
  MicroProfileWSPrintf("{\"k\":\"%d\",\"v\":{\"id\":%d,\"pid\":%d,",
                       MSG_TIMER_TREE, id, parent);
  MicroProfileWSPrintf("\"name\":\"%s\",", pName);
  MicroProfileWSPrintf("\"e\":%d,", nEnabled);
  if (nColor == 0x42) {
    MicroProfileWSPrintf("\"color\":\"\"");
  } else {
    MicroProfileWSPrintf("\"color\":\"#%02x%02x%02x\"",
                         MICROPROFILE_UNPACK_RED(nColor) & 0xff,
                         MICROPROFILE_UNPACK_GREEN(nColor) & 0xff,
                         MICROPROFILE_UNPACK_BLUE(nColor) & 0xff);
  }
...
void MicroProfileWebSocketSendCounterEntry(uint32_t id, uint32_t parent,
                                           const char *pName, int64_t nLimit,
                                           int nFormat) {
  MicroProfileWSPrintf("{\"k\":\"%d\",\"v\":{\"id\":%d,\"pid\":%d,",
                       MSG_TIMER_TREE, id, parent);
  MicroProfileWSPrintf("\"name\":\"%s\",", pName);
...
void MicroProfileWebSocketSendState(MpSocket C) {
  if (S.WSCategoriesSent != S.nCategoryCount ||
      S.WSGroupsSent != S.nGroupCount || S.WSTimersSent != S.nTotalTimers ||
    ...
    for (uint32_t i = S.WSGroupsSent; i < S.nGroupCount; ++i) {
      ...
      uint32_t id = MicroProfileWebSocketIdPack(TYPE_GROUP, i);
      uint32_t parent =
          MicroProfileWebSocketIdPack(TYPE_CATEGORY, GI.nCategory);
      MicroProfileWebSocketSendEntry(id, parent, GI.pName,
                                     MicroProfileGroupEnabled(i), GI.nColor);
    }
    ...
    for (uint32_t i = S.WSTimersSent; i < S.nTotalTimers; ++i) {
      ...
      MicroProfileWebSocketSendEntry(id, parent, TI.pName,
                                     MicroProfileWebSocketTimerEnabled(i),
                                     TI.nColor);
    }
    ...
    for (uint32_t i = S.WSCountersSent; i < S.nNumCounters; ++i) {
      ...
      MicroProfileWebSocketSendCounterEntry(id, parent, CI.pName, CI.nLimit,
                                            CI.eFormat);
    }
#if MICROPROFILE_CONTEXT_SWITCH_TRACE
    MicroProfileWebSocketSendEntry(
        MicroProfileWebSocketIdPack(TYPE_SETTING, SETTING_CONTEXT_SWITCH_TRACE),
        0, "Context Switch Trace", S.bContextSwitchRunning, (uint32_t)-1);
#endif
      ...
      if (pHttp && pGet) {
        *pHttp = '\0';
        ...
        case EMICROPROFILE_GET_COMMAND_LIVE: {
          ...
          MicroProfileDumpHtmlLive(MicroProfileCompressedWriteSocket,
                                   &CompressState);
        ...
        case EMICROPROFILE_GET_COMMAND_DUMP: {
          {
            ...
            MicroProfileDumpHtml(MicroProfileCompressedWriteSocket,
                                 &CompressState, R.nFrames, pHost,
                                 R.nFrameStart);
            ...
            uint64_t nTickEnd = MP_TICK();
            uint64_t nDiff = (nTickEnd - nTickStart);
            float fMs = MicroProfileTickToMsMultiplier(
                            MicroProfileTicksPerSecondCpu()) *
                        nDiff;
            int nKb = ((nDataEnd - nDataStart) >> 10) + 1;
            int nCompressedKb = ((CompressState.nCompressedSize) >> 10) + 1;
...
void MicroProfileContextSwitchShutdownTrace() {
  ...
  log.LoggerName = const_cast<LPWSTR>(KERNEL_LOGGER_NAME);
  log.ProcessTraceMode = 0;
  TRACEHANDLE hLog = OpenTrace(&log);
  if (hLog) {
    ControlTrace(SessionHandle, KERNEL_LOGGER_NAME, &sessionProperties,
...
bool MicroProfileStartWin32Trace(EventCallback EvtCb, BufferCallback BufferCB) {
  ...
  sessionProperties.BufferSize = 1;
  sessionProperties.NumberOfBuffers = 128;
  sessionProperties.EnableFlags =
      EVENT_TRACE_FLAG_CSWITCH | EVENT_TRACE_FLAG_PROCESS;
  sessionProperties.LogFileMode = EVENT_TRACE_REAL_TIME_MODE;
  sessionProperties.MaximumFileSize = 0;
  sessionProperties.LoggerNameOffset = sizeof(EVENT_TRACE_PROPERTIES);
  sessionProperties.LogFileNameOffset = 0;
...
const char *MicroProfileWin32ThreadInfoAddString(const char *pString) {
  ...
  for (uint32_t i = 0; i < MAX_SEARCH; ++i) {
    ...
    if (0 == g_ThreadInfo.pStrings[idx]) {
      g_ThreadInfo.pStrings[idx] =
          &g_ThreadInfo.StringData[g_ThreadInfo.nStringOffset];
      memcpy(&g_ThreadInfo.StringData[g_ThreadInfo.nStringOffset], pString,
             nLen + 1);
...
void MicroProfileWin32InitThreadInfo2() {
    ...
    if (Process32First(hSnap, &pe32)) {
      do {
        ...
        g_ThreadInfo.P[g_ThreadInfo.nNumProcesses++] = P;
               ...
               g_ThreadInfo.nNumProcesses <
                   MicroProfileWin32ThreadInfo::MAX_PROCESSES);
    }
#if MICROPROFILE_DEBUG
    int64_t nTicksEnd = MP_TICK();
  ...
  }
  {
    ...
    uprintf("Module iteration %6.2fms NumModules %d\n", fMs,
            g_ThreadInfo.nNumModules);
#endif
  }

  pNtQIT NtQueryInformationThread = (pNtQIT)GetProcAddress(
    ...
    do {
      ...
      if (hThread) {
        ...
        NTSTATUS ntStatus = NtQueryInformationThread(
            ...
            &dwStartAddress, sizeof(dwStartAddress), &olen);
        if (0 == ntStatus) {
        ...
        CloseHandle(hThread);
      {
        ...
        g_ThreadInfo.T[g_ThreadInfo.nNumThreads++] = T;
             ...
             g_ThreadInfo.nNumThreads <
                 MicroProfileWin32ThreadInfo::MAX_THREADS);
...
#define MICROPROFILE_WIN32_CSWITCH_TIMEOUT                                     \
  15 // seconds to wait before collector exits
    ...
    if (pEvent->Header.Class.Type == 36) {
      ...
      if ((pCSwitch->NewThreadId != 0) || (pCSwitch->OldThreadId != 0)) {
        ...
        if (nPut - nGet < MicroProfileWin32ContextSwitchShared::BUFFER_SIZE) {
          g_pShared->Buffer[nPut %
                            MicroProfileWin32ContextSwitchShared::BUFFER_SIZE] =
              Switch;
...
int main(int argc, char *argv[]) {
  ...
  g_pShared = (MicroProfileWin32ContextSwitchShared *)MapViewOfFile(
      hMemory, FILE_MAP_ALL_ACCESS, 0, 0,
      sizeof(MicroProfileWin32ContextSwitchShared));

  if (g_pShared != NULL) {
    MicroProfileStartWin32Trace(MicroProfileContextSwitchCallbackCollector,
...
void *MicroProfileTraceThread(void *unused) {
  ...
  MicroProfileContextSwitchShutdownTrace();
  if (!MicroProfileStartWin32Trace(MicroProfileContextSwitchCallback,
                                   MicroProfileBufferCallback)) {
    MicroProfileContextSwitchShutdownTrace();
    // not running as admin. try and start other process.
    MicroProfileWin32ContextSwitchShared *pShared = 0;
    char Filename[512];
    time_t t = time(NULL);
    _snprintf_s(Filename, sizeof(Filename), "%s_%d", MICROPROFILE_FILEMAPPING,
                (int)t);
    ...
    if (hMemory != NULL) {
      pShared = (MicroProfileWin32ContextSwitchShared *)MapViewOfFile(
          hMemory, FILE_MAP_ALL_ACCESS, 0, 0,
          sizeof(MicroProfileWin32ContextSwitchShared));
      if (pShared != NULL) {
#ifdef _M_IX86
...
#define CSWITCH_EXE "microprofile-win32-cswitch_x64.exe"
        ...
        HINSTANCE Instance = ShellExecuteA(NULL, "runas", CSWITCH_EXE, Filename,
                                           "", SW_SHOWMINNOACTIVE);
            ...
            if (nPut == nGet) {
              ...
              for (int64_t i = nGet; i != nPut; i++) {
                MicroProfileContextSwitchPut(
                    &pShared->Buffer[i % MicroProfileWin32ContextSwitchShared::
                                             BUFFER_SIZE]);
              }
...
MicroProfileGetThreadInfo(MicroProfileThreadIdType nThreadId) {
  MicroProfileWin32UpdateThreadInfo();
...
void *MicroProfileTraceThread(void *unused) {
  ...
  if (!pFile) {
    uprintf(
        "CONTEXT SWITCH FAILED TO OPEN FILE: make sure to run dtrace script\n");
    S.bContextSwitchRunning = false;
    return 0;
  }
...
uint32_t MicroProfileGpuInsertTimeStamp(void *pContext_) {
  if (S.pGPU) {
    MicroProfileD3D11Frame &Frame =
        S.pGPU->m_QueryFrames[S.pGPU->m_nQueryFrame];
...
uint32_t MicroProfileGpuFlip(void *pDeviceContext_) {
  ...
  MicroProfileD3D11Frame &CurrentFrame =
      S.pGPU->m_QueryFrames[S.pGPU->m_nQueryFrame];
  ...
  uint32_t nNextFrame =
      (S.pGPU->m_nQueryFrame + 1) % MICROPROFILE_GPU_FRAME_DELAY;
  S.pGPU->m_nQueryPut =
      (CurrentFrame.m_nQueryStart + CurrentFrame.m_nQueryCount) %
      MICROPROFILE_D3D_MAX_QUERIES;
  ...
  if (OldFrame.m_nRateQueryStarted) {
    ...
    if (MicroProfileGpuGetData(OldFrame.m_pRateQuery, &Result,
                               sizeof(Result))) {
    ...
    S.pGPU->m_nQueryGet = (OldFrame.m_nQueryStart + OldFrame.m_nQueryCount) %
                          MICROPROFILE_D3D_MAX_QUERIES;
  }
  ...
  if (S.pGPU->m_nQueryPut >= S.pGPU->m_nQueryGet) {
    NextFrame.m_nQueryCountMax =
        (MICROPROFILE_D3D_MAX_QUERIES - S.pGPU->m_nQueryPut) +
        S.pGPU->m_nQueryGet;
  } else {
...
void MicroProfileGpuInitD3D11(void *pDevice_, void *pImmediateContext) {
  ...
  for (uint32_t i = 0; i < MICROPROFILE_D3D_MAX_QUERIES; ++i) {
    HRESULT hr =
        pDevice->CreateQuery(&Desc, (ID3D11Query **)&S.pGPU->m_pQueries[i]);
    MP_ASSERT(hr == S_OK);
  ...
  for (uint32_t i = 0; i < MICROPROFILE_GPU_FRAME_DELAY; ++i) {
    ...
    hr = pDevice->CreateQuery(
        &Desc, (ID3D11Query **)&S.pGPU->m_QueryFrames[i].m_pRateQuery);
    MP_ASSERT(hr == S_OK);
  }
}
...
void MicroProfileGpuShutdown() {
  ...
  for (uint32_t i = 0; i < MICROPROFILE_GPU_FRAME_DELAY; ++i) {
    if (S.pGPU->m_QueryFrames[i].m_pRateQuery) {
      ID3D11Query *pQuery =
          (ID3D11Query *)S.pGPU->m_QueryFrames[i].m_pRateQuery;
      pQuery->Release();
...
int MicroProfileGetGpuTickReference(int64_t *pOutCPU, int64_t *pOutGpu) {
  if (S.pGPU) {
    ...
    if (Frame.m_nRateQueryStarted) {
      ...
      HRESULT hr;
      do {
        hr = pImmediateContext->GetData(pSyncQuery, pOutGpu, sizeof(*pOutGpu),
                                        0);
      } while (hr == S_FALSE);
      *pOutCPU = MP_TICK();
      switch (hr) {
...
uint32_t MicroProfileGpuInsertTimeStamp(void *pContext) {
  ...
  uint32_t nQueryIndex =
      (S.pGPU->nFrameCount.fetch_add(1) + S.pGPU->nFrameStart) %
      MICROPROFILE_D3D_MAX_QUERIES;

  ID3D12GraphicsCommandList *pCommandList =
      (ID3D12GraphicsCommandList *)pContext;
  pCommandList->EndQuery(S.pGPU->NodeState[nNode].pHeap,
                         D3D12_QUERY_TYPE_TIMESTAMP, nQueryIndex);
...
void MicroProfileGpuFetchRange(uint32_t nBegin, int32_t nCount, uint64_t nFrame,
                               int64_t nTimestampOffset) {
  if (nCount <= 0)
    return;
  void *pData = 0;
  // uprintf("fetch [%d-%d]\n", nBegin, nBegin + nCount);
  D3D12_RANGE Range = {sizeof(uint64_t) * nBegin,
                       sizeof(uint64_t) * (nBegin + nCount)};
  S.pGPU->pBuffer->Map(0, &Range, &pData);
  memcpy(&S.pGPU->nResults[nBegin], nBegin + (uint64_t *)pData,
...
void MicroProfileGpuWaitFence(uint32_t nNode, uint64_t nFence) {
  uint64_t nCompletedFrame =
      S.pGPU->NodeState[nNode].pFence->GetCompletedValue();
...
uint64_t MicroProfileTicksPerSecondGpu() { return S.pGPU->nFrequency; }
...
uint32_t MicroProfileGpuFlip(void *pContext) {
  ...
  ID3D12GraphicsCommandList *pCommandList =
      S.pGPU->Frames[nFrameIndex].pCommandList[nNode];
  ID3D12CommandAllocator *pCommandAllocator =
      S.pGPU->Frames[nFrameIndex].pCommandAllocator;
  ...
  if (S.pGPU->nFrameCount) {
    ...
    S.pGPU->nFrameStart =
        (S.pGPU->nFrameStart + nCount) % MICROPROFILE_D3D_MAX_QUERIES;
    uint32_t nEnd = MicroProfileMin(nStart + nCount,
                                    (uint32_t)MICROPROFILE_D3D_MAX_QUERIES);
    MP_ASSERT(nStart != nEnd);
    uint32_t nSize = nEnd - nStart;
    pCommandList->ResolveQueryData(
        S.pGPU->NodeState[nNode].pHeap, D3D12_QUERY_TYPE_TIMESTAMP, nStart,
        nEnd - nStart, S.pGPU->pBuffer, nStart * sizeof(int64_t));
    if (nStart + nCount > MICROPROFILE_D3D_MAX_QUERIES) {
      pCommandList->ResolveQueryData(
          S.pGPU->NodeState[nNode].pHeap, D3D12_QUERY_TYPE_TIMESTAMP, 0,
          nEnd + nStart - MICROPROFILE_D3D_MAX_QUERIES, S.pGPU->pBuffer, 0);
    }
  }
  pCommandList->Close();
...
void MicroProfileGpuInitD3D12(void *pDevice_, uint32_t nNodeCount,
                              void **pCommandQueues_) {
  ...
  for (uint32_t nNode = 0; nNode < S.pGPU->nNodeCount; ++nNode) {
    S.pGPU->NodeState[nNode].pCommandQueue =
        (ID3D12CommandQueue *)pCommandQueues_[nNode];
    if (nNode == 0) {
    ...
    HRESULT hr = pDevice->CreateQueryHeap(
        &QHDesc, IID_PPV_ARGS(&S.pGPU->NodeState[nNode].pHeap));
    MP_ASSERT(hr == S_OK);
  ...
  for (MicroProfileD3D12Frame &Frame : S.pGPU->Frames) {
    ...
    for (uint32_t nNode = 0; nNode < S.pGPU->nNodeCount; ++nNode) {
      hr = pDevice->CreateCommandList(MP_NODE_MASK_ONE(nNode),
                                      ...
                                      Frame.pCommandAllocator, nullptr,
...
uint32_t MicroProfileGpuInsertTimeStamp(void *pContext) {
  ...
  vkCmdWriteTimestamp(CB, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
                      S.pGPU->QueryPool[nNode], nQueryIndex);
  MP_ASSERT(nQueryIndex <= 0xffff);
  // uprintf("insert timestamp %d :: %d ... ctx %p\n", nQueryIndex, nFrame,
  // pContext);
  return ((nFrame << 16) & 0xffff0000) | (nQueryIndex);
...
void MicroProfileGpuFetchRange(VkCommandBuffer CommandBuffer, uint32_t nNode,
                               uint32_t nBegin, int32_t nCount, uint64_t nFrame,
                               int64_t nTimestampOffset) {
  if (nCount <= 0)
    return;

  vkGetQueryPoolResults(S.pGPU->Devices[nNode], S.pGPU->QueryPool[nNode],
                        nBegin, nCount, 8 * nCount, &S.pGPU->nResults[nBegin],
                        8,
...
void MicroProfileGpuWaitFence(uint32_t nNode, uint64_t nFrame) {
  ...
  do {
    MICROPROFILE_SCOPEI("Microprofile", "gpu-wait", MP_GREEN4);
    r = vkWaitForFences(S.pGPU->Devices[nNode], 1,
                        &S.pGPU->Frames[nFrame].Fences[nNode], 1, 1000 * 30);
#if 0
		if(c++ > 1000 && (c%100) == 0)
		{
...
uint32_t MicroProfileGpuFlip(void *pContext) {
  ...
  VkCommandBuffer CommandBuffer =
      S.pGPU->Frames[nFrameIndex].CommandBuffer[nNode];
  ...
  vkResetCommandBuffer(F.CommandBuffer[nNode],
  ...
  nStart = S.pGPU->nFrameStart;
  S.pGPU->nFrameStart =
      (S.pGPU->nFrameStart + nCount) % MICROPROFILE_VULKAN_MAX_QUERIES;
  uint32_t nEnd = MicroProfileMin(nStart + nCount,
                                  (uint32_t)MICROPROFILE_VULKAN_MAX_QUERIES);
  MP_ASSERT(nStart != nEnd);
  uint32_t nSize = nEnd - nStart;
  ...
  S.pGPU->nFrame++;
  ////fetch from earlier frames
  MicroProfileGpuFetchResults(CommandBuffer,
                              S.pGPU->nFrame - MICROPROFILE_GPU_FRAME_DELAY);
...
void MicroProfileGpuInitVulkan(VkDevice *pDevices,
                               VkPhysicalDevice *pPhysicalDevices,
                               VkQueue *pQueues, uint32_t *QueueFamily,
                               uint32_t nNodeCount) {
  S.pGPU = MP_ALLOC_OBJECT(MicroProfileGpuTimerState);
  memset(S.pGPU, 0, sizeof(*S.pGPU));
  S.pGPU->nNodeCount = nNodeCount;
  ...
  for (uint32_t i = 0; i < nNodeCount; ++i) {
    ...
    r = vkCreateCommandPool(S.pGPU->Devices[i], &CreateInfo, 0,
                            &S.pGPU->CommandPool[i]);
    MP_ASSERT(r == VK_SUCCESS);
    ...
    for (uint32_t j = 0; j < MICROPROFILE_VULKAN_INTERNAL_DELAY; ++j) {
      ...
      r = vkAllocateCommandBuffers(S.pGPU->Devices[i], &AllocInfo,
                                   &F.CommandBuffer[i]);
      MP_ASSERT(r == VK_SUCCESS);
      ...
      if (j == 0) {
        ...
        vkResetCommandBuffer(F.CommandBuffer[i],
                             VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
      }
    }
  }
...
int MicroProfileGetGpuTickReference(int64_t *pOutCPU, int64_t *pOutGpu) {
  ...
  vkCmdResetQueryPool(CB, S.pGPU->QueryPool[nGpu],
                      MICROPROFILE_VULKAN_MAX_QUERIES, 1);
  vkCmdWriteTimestamp(CB, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
                      S.pGPU->QueryPool[nGpu], MICROPROFILE_VULKAN_MAX_QUERIES);
  ...
  vkWaitForFences(Device, 1, &Fence, 1, (uint64_t)-1);
  *pOutGpu = 0;
  vkGetQueryPoolResults(Device, S.pGPU->QueryPool[nGpu],
                        MICROPROFILE_VULKAN_MAX_QUERIES, 1, 8, pOutGpu, 8,
...
uint64_t MicroProfileTicksPerSecondGpu() { return 1000000000ll; }
...
uint32_t MicroProfileStringHash(
    const char *pString) // note matching: code in javascript:
                         // microprofilelive.html: function StringHash(s)
{
...
bool MicroProfileCopyInstructionBytes(char *pDest, void *pSrc, const int nLimit,
                                      const int nMaxSize, char *pTrunk,
                                      intptr_t nTrunkSize,
                                      uint32_t nUsableJumpRegs, int *nBytesDest,
                                      int *nBytesSrc, uint32_t *pRegsWritten,
                                      uint32_t *nRetSafe);
bool MicroProfilePatchFunction(void *f, int Argument,
                               MicroProfileHookFunc enter,
                               MicroProfileHookFunc leave,
                               MicroProfilePatchError *pError);
template <typename Callback>
void MicroProfileIterateSymbols(Callback CB, uint32_t *nModules,
                                uint32_t nNumModules);
...
typedef void (*MicroProfileOnSymbolCallback)(const char *pSymbolName,
                                             intptr_t nAddress);
...
uint8_t *MicroProfileInsertMov(uint8_t *p, uint8_t *pend, int r,
                               intptr_t value) {
...
bool MicroProfileStringMatch(const char *pSymbol, uint32_t nStartOffset,
                             const char **pPatterns, uint32_t *nPatternLength,
                             uint32_t nNumPatterns) {
...
int MicroProfileStringMatchOffset(const char *pSymbol, const char **pPatterns,
                                  uint32_t *nPatternLength,
                                  uint32_t nNumPatterns) {
  int nOffset = 0;
  const char *p = pSymbol;
...
void *MicroProfileX64FollowJump(void *pSrc) {
  for (uint32_t i = 0; i < S.DynamicTokenIndex; ++i)
    if (S.FunctionsInstrumented[i] == pSrc)
      return pSrc; // if already instrumented, do not follow the jump inserted
                   // by itself.
...
bool MicroProfileCopyInstructionBytes(
    char *pDest, void *pSrc, const int nLimit, const int nMaxSize, char *pTrunk,
    intptr_t nTrunkSize, const uint32_t nUsableJumpRegs, int *pBytesDest,
    int *pBytesSrc, uint32_t *pRegsWritten, uint32_t *pRetSafe) {
...
#ifdef _WIN32
  const uint32_t nUsableRegisters =
      RegToBit(R_RAX) | RegToBit(R_R10) | RegToBit(R_R11);
#else
  const uint32_t nUsableRegisters =
      RegToBit(R_RAX) | RegToBit(R_R10) | RegToBit(R_R11);
#endif
  ...
  for (i = 0; i < nCount; ++i) {
    ...
    if (I.opcode == I_CALL) {
      ...
      if ((nRegsWritten & nUsableRegisters) == nUsableRegisters) {
        uprintf("call encountered, but register all regs was written to. TODO: "
                "push regs?\n");
        BREAK_ON_PATCH_FAIL();
        return false;
      }
    ...
    case O_REG: {
      ...
      case O_MEM:
      case O_SMEM: {
        // if register is RSP 'contaminated', it prevents us from using that to
        // do retjmps
...
extern "C" void MicroProfileInterceptEnter(int a) {
  ...
  MicroProfileThreadLog *pLog = MicroProfileGetThreadLog2();
  MP_ASSERT(pLog->nStackScope <
            MICROPROFILE_STACK_MAX); // if youre hitting this assert your
                                     // instrumenting a deeply nested function
...
uint32_t MicroProfileColorFromString(
    const char *pString) // note matching code/constants in javascript:
                         // microprofilelive.html: function StringToColor(s)
{
  // 	var h = StringHash(s);
  // var cidx = h % 360;
  // return "hsl(" + cidx + ",50%, 70%)"; //note: matching code constants in
  // microprofile.cpp: MicroProfileColorFromString
...
void MicroProfileInstrumentFromAddressOnly(void *pFunction) {
  ...
  if (pDesc) {
    uprintf("Found function %p :: %s %s\n", (void *)pDesc->nAddress,
            pDesc->pName, pDesc->pShortName);
    uint32_t nColor = MicroProfileColorFromString(pDesc->pName);

    MicroProfileInstrumentFunction(
        pFunction, MicroProfileSymbolModuleGetString(pDesc->nModule),
        pDesc->pName, nColor);
  } else {
...
void MicroProfileInstrumentFunctionsCalled(void *pFunction,
  ...
  if (pDesc) {
    uprintf("instrumenting child functions %p %p :: %s :: %s\n",
            (void *)pDesc->nAddress, (void *)pDesc->nAddressEnd, pDesc->pName,
            pDesc->pShortName);
    int a = 0;
    (void)a;
  } else {
  ...
  const intptr_t nCodeLen =
      (intptr_t)pDesc->nAddressEnd - (intptr_t)pDesc->nAddress;
...
void MicroProfileInstrumentFunction(void *pFunction, const char *pModuleName,
  ...
  if (MicroProfilePatchFunction(pFunction, S.DynamicTokenIndex,
                                MicroProfileInterceptEnter,
                                MicroProfileInterceptLeave, &Err)) {
    MicroProfileToken Tok = S.DynamicTokens[S.DynamicTokenIndex] =
        MicroProfileGetToken("PATCHED", pFunctionName, nColor,
                             MicroProfileTokenTypeCpu);
    S.FunctionsInstrumented[S.DynamicTokenIndex] = pFunction;
    S.FunctionsInstrumentedName[S.DynamicTokenIndex] =
        MicroProfileStringIntern(pFunctionName);
    S.FunctionsInstrumentedModuleNames[S.DynamicTokenIndex] =
        MicroProfileStringIntern(pModuleName);
    S.FunctionsInstrumentedUnmangled[S.DynamicTokenIndex] =
        MicroProfileGetUnmangledSymbolName(pFunction);
    S.DynamicTokenIndex++;
    ...
    for (int i = 0; i < S.nNumPatchErrors; ++i) {
      if (Err.nCodeSize == S.PatchErrors[i].nCodeSize &&
          0 == memcmp(Err.Code, S.PatchErrors[i].Code, Err.nCodeSize)) {
        bFound = true;
        break;
      }
    }
...
bool MicroProfileSymbolInitialize(bool bStartLoad, const char *pModuleName) {
  ...
  // int nRequests = 0;
  {
    ...
    for (int i = 0; i < S.SymbolNumModules; ++i) {
      if (0 == pModuleName ||
          0 == strcmp(pModuleName,
                      (const char *)S.SymbolModules[i].pBaseString)) {
  ...
  return S.SymbolState.nModuleLoadsRequested.load() ==
...
void MicroProfileSymbolFreeDataInternal() {
  {
    ...
    for (int i = 0; i < S.SymbolNumModules; ++i) {
      ...
      while (S.SymbolModules[i].pSymbolBlock) {
        ...
        MICROPROFILE_COUNTER_SUB("/MicroProfile/Symbols/Memory",
                                 sizeof(MicroProfileSymbolBlock));
      }
    }
...
void MicroProfileCharacterMaskString2(const char *pStr,
                                      MicroProfileStringMatchMask &M) {
  uint_string_match nMask = 0;
  char c = 0;
  int nLast = -1;
  while (0 != (c = *pStr++)) {
...
bool MicroProfileCharacterMatch(const MicroProfileStringMatchMask &Block,
                                const MicroProfileStringMatchMask &String) {
...
void MicroProfileSymbolMergeExecutableRegions() {
  for (int i = 0; i < S.SymbolNumModules; ++i) {
    ...
    if (M.nNumExecutableRegions > 1) {
      std::sort(&M.Regions[0], &M.Regions[M.nNumExecutableRegions],
                [](const MicroProfileSymbolModuleRegion &l,
                   const MicroProfileSymbolModuleRegion &r) {
  ...
  for (int i = 0; i < S.SymbolNumModules; ++i) {
    ...
    for (int j = 0; j < M.nNumExecutableRegions; ++j)
      uprintf("\t[%p-%p]\n", (void *)M.Regions[j].nBegin,
              (void *)M.Regions[j].nEnd);
  }
}

uint32_t MicroProfileSymbolInitModule(const char *pString_, intptr_t nAddrBegin,
                                      intptr_t nAddrEnd) {
  ...
  for (int i = 0; i < S.SymbolNumModules; ++i) {
    ...
    if (M.pBaseString == pString) {
      ...
      // (void*)nAddrBegin, (void*)nAddrEnd, pString);
      M.nNumExecutableRegions++;
      return i;
    }
  }
  ...
  if (S.SymbolModuleNameOffset + nLen >
      MICROPROFILE_INSTRUMENT_MAX_MODULE_CHARS)
    return 0;
  memcpy(S.SymbolModuleNameOffset + &S.SymbolModuleNameBuffer[0],
         pTrimmedString, nLen);
...
void MicroProfileSymbolInitializeInternal() {
  ...
  auto AllocBlock = []() -> MicroProfileSymbolBlock * {
    ...
    MICROPROFILE_COUNTER_ADD("/MicroProfile/Symbols/Memory",
                             sizeof(MicroProfileSymbolBlock));
    MICROPROFILE_COUNTER_CONFIG_ONCE("/MicroProfile/Symbols/Memory",
                                     MICROPROFILE_COUNTER_FORMAT_BYTES, 0, 0);
    memset(pBlock, 0, sizeof(MicroProfileSymbolBlock));
    return pBlock;
  };

  auto SymbolCallback = [&](const char *pName, const char *pShortName,
                            intptr_t nAddress, intptr_t nAddressEnd,
                            uint32_t nModuleId) {
    ...
    if (strstr(pName, "MicroProfile")) {
      ...
      nIgnoreSymbol = 1;
#else
      if (strstr(pName, "Log") || strstr(pName, "Scope") ||
          strstr(pName, "Tick") || strstr(pName, "Enter") ||
          strstr(pName, "Leave") || strstr(pName, "Thread") ||
          strstr(pName, "Thread") ||
          strstr(pName, "Mutex")) // just for debugging: skip these so we can
                                  // play around with the sample projects
      {
        nIgnoreSymbol = 1;
      }
#endif
    ...
    if (strstr(pName, "__security_check_cookie") ||
        strstr(pName, "_RTC_CheckStackVars") || strstr(pName, "__chkstk") ||
        strstr(pName, "std::_Atomic")) {
      nIgnoreSymbol = 1;
    }
#endif
    MicroProfileSymbolBlock *pActiveBlock =
        S.SymbolModules[nModule].pSymbolBlock;
    if (!pActiveBlock) {
    ...
    char *pStr = &pActiveBlock->Chars[MicroProfileSymbolBlock::ESIZE -
                                      pActiveBlock->nNumChars - 1];
    memcpy(pStr, pName, nLen);
    pStr[nLen - 1] = '\0';
    MicroProfileSymbolDesc &E =
        pActiveBlock->Symbols[pActiveBlock->nNumSymbols++];
    E.pName = pStr;
    E.nAddress = nAddress;
    ...
    if (pShortName && strlen(pShortName)) {
      ...
      char *pStrShort = &pActiveBlock->Chars[MicroProfileSymbolBlock::ESIZE -
                                             pActiveBlock->nNumChars - 1];
...
#define SYMDBG 0
    ...
    uprintf("Got symbol %lld %lld %f .. %llx    %llx    %llx %s\n",
            ...
            S.SymbolModules[nModule].nProgressTarget
                ? float(S.SymbolModules[nModule].nProgress) /
                      float(S.SymbolModules[nModule].nProgressTarget)
                : 0.f,
            (int64_t)E.nAddress, (int64_t)S.SymbolModules[nModule].nAddrBegin,
            (int64_t)S.SymbolModules[nModule].nAddrEnd, E.pName);
    if (E.nAddress < (int64_t)S.SymbolModules[nModule].nAddrBegin ||
        E.nAddress > (int64_t)S.SymbolModules[nModule].nAddrEnd) {
      MP_BREAK();
    }
#endif
...
#undef SYMDBG
    ...
    MP_ASSERT((intptr_t)E.pShortName >=
              (intptr_t)&E); // assert pointer arithmetic is correct.
  };
  do {
    uint32_t nModuleLoad[MICROPROFILE_INSTRUMENT_MAX_MODULES];
    uint32_t nNumModulesRequested = 0;
    for (int i = 0; i < S.SymbolNumModules; ++i) {
      if (S.SymbolModules[i].nModuleLoadRequested.load() != 0 &&
          S.SymbolModules[i].nModuleLoadFinished.load() == 0) {
    ...
    MicroProfileIterateSymbols(SymbolCallback, nModuleLoad,
                               nNumModulesRequested);
    S.SymbolState.nModuleLoadsFinished.fetch_add(nNumModulesRequested);
    for (uint32_t i = 0; i < nNumModulesRequested; ++i) {
      if (S.SymbolModules[nModuleLoad[i]].nModuleLoadRequested.load() ==
          S.SymbolModules[nModuleLoad[i]].nModuleLoadFinished.load()) {
        S.SymbolModules[nModuleLoad[i]].nProgress =
            S.SymbolModules[nModuleLoad[i]].nProgressTarget;
        S.nSymbolsDirty.exchange(1);
      }
    }
  } while (1);
...
struct MicroProfileFunctionQuery {
  ...
  uint32_t nModuleFilterMatch
      [MICROPROFILE_INSTRUMENT_MAX_MODULES]; // prematch the modules, so it can
                                             // be skipped during search
  uint32_t nMask[MICROPROFILE_MAX_FILTER];   // masks for subpatterns skipped
  MicroProfileStringMatchMask
      MatchMask[MICROPROFILE_MAX_FILTER]; // masks for subpatterns skipped
...
MicroProfileFunctionQuery *MicroProfileAllocFunctionQuery() {
  ...
  if (S.pQueryFreeList != 0) {
    ...
    MICROPROFILE_COUNTER_ADD("MicroProfile/Symbols/FunctionQueryMem",
                             sizeof(MicroProfileFunctionQuery));
    S.nNumQueryAllocated++;
  }
...
void MicroProfileProcessQuery(MicroProfileFunctionQuery *pQuery) {
  ...
  for (int i = 0; i < S.SymbolNumModules; ++i) {
    ...
    MicroProfileStringMatchMask &MatchMaskQ = Q.MatchMask[nModuleMatchOffset];
    {
      while (pSymbols && 0 == S.pPendingQuery &&
             Q.nNumResults < MICROPROFILE_MAX_QUERY_RESULTS) {
        ...
        if (MicroProfileCharacterMatch(pSymbols->MatchMask, MatchMaskQ)) {
          nBlocksTested++;
          for (uint32_t i = 0;
               i < pSymbols->nNumSymbols && 0 == S.pPendingQuery &&
               Q.nNumResults < MICROPROFILE_MAX_QUERY_RESULTS;
               ++i) {
            MicroProfileSymbolDesc &E = pSymbols->Symbols[i];
            if (0 == E.nIgnoreSymbol) {
              ...
              if (nMaskQ == (nMaskQ & E.nMask)) {
                ...
                if (MicroProfileStringMatch(E.pShortName, nModuleMatchOffset,
                                            &Q.pFilterStrings[0],
                                            Q.nPatternLength, Q.nMaxFilter)) {
  ...
  (void)TIME;
  (void)TIME0;
  uprintf(" %6.3fms [%6.3f]: %5d/%5d blocks tested. %5d symbols %5d/%5d string "
          "compares\n",
          TIME, TIME0, nBlocksTested, nBlocks, nSymbolsTested, nStringsTested,
          nStringsTested0);
}
...
void *MicroProfileQueryThread(void *p) {
  MicroProfileOnThreadCreate("MicroProfileSymbolThread");
  {
    while (1) {
      ...
      if (S.SymbolState.nModuleLoadsRequested.load() !=
          S.SymbolState.nModuleLoadsFinished.load()) {
        L.Unlock();
...
void MicroProfileSymbolSendFunctionNames(MpSocket Connection) {
  if (S.WSFunctionsInstrumentedSent < S.DynamicTokenIndex) {
    ...
    bool bFirst = true;
    for (uint32_t i = S.WSFunctionsInstrumentedSent; i < S.DynamicTokenIndex;
         ++i) {
      ...
      MicroProfileWSPrintf(bFirst ? "[\"%s\",\"%s\",\"%s\"]"
                                  : ",[\"%s\",\"%s\",\"%s\"]",
                           pString, pModuleString, pUnmangled);
      bFirst = false;
    }
    MicroProfileWSPrintf("]}");
    MicroProfileWSFlush();
...
void MicroProfileSymbolSendErrors(MpSocket Connection) {
  if (S.nNumPatchErrors) {
    MicroProfileWSPrintStart(Connection);
    MicroProfileWSPrintf(
        "{\"k\":\"%d\",\"v\":{\"version\":\"%d.%d\",\"data\":[",
        MSG_INSTRUMENT_ERROR, MICROPROFILE_MAJOR_VERSION,
        MICROPROFILE_MINOR_VERSION);
    bool bFirst = true;
    for (int i = 0; i < S.nNumPatchErrors; ++i) {
      ...
      MicroProfileWSPrintf("\",\"message\":\"%s\",\"already\":%d}",
                           &E.Message[0], E.AlreadyInstrumented);
      bFirst = false;
    }
...
void MicroProfileSymbolQuerySendResult(MpSocket Connection) {
  MICROPROFILE_SCOPEI("MicroProfile", "MicroProfileSymbolQuerySendResult",
                      MP_PINK2);
  ...
  if (pQuery) {
    ...
    MicroProfileWSPrintStart(Connection);
    MicroProfileWSPrintf("{\"k\":\"%d\",\"q\":%d,\"v\":[", MSG_FUNCTION_RESULTS,
                         pQuery->QueryId);
    bool bFirst = true;
    for (uint32_t i = 0; i < pQuery->nNumResults; ++i) {
      ...
      if (bFirst) {
        MicroProfileWSPrintf(
            "{\"a\":\"%p\",\"n\":\"%s\",\"sn\":\"%s\",\"m\":\"%s\"}",
            E.nAddress, E.pName, E.pShortName,
            MicroProfileSymbolModuleGetString(E.nModule));
        bFirst = false;
      } else {
        MicroProfileWSPrintf(
            ",{\"a\":\"%p\",\"n\":\"%s\",\"sn\":\"%s\",\"m\":\"%s\"}",
...
void MicroProfileSymbolQueryFunctions(MpSocket Connection,
                                      const char *pFilter) {
  MICROPROFILE_SCOPEI("MicroProfile", "MicroProfileSymbolQueryFunctions",
                      MP_WHEAT);
  ...
  }
  {
    ...
    if (0 == S.pPendingQuery || S.pPendingQuery->QueryId < (uint32_t)QueryId) {
      ...
      for (int i = 0; i < S.SymbolNumModules; ++i) {
        Q.nModuleFilterMatch[i] = MicroProfileStringMatchOffset(
            MicroProfileSymbolModuleGetString(i), Q.pFilterStrings,
            Q.nPatternLength, Q.nMaxFilter);
      }
...
bool MicroProfilePatchFunction(void *f, int Argument,
  ...
  intptr_t t_enter_patch_offset =
      (intptr_t)microprofile_tramp_enter_patch - t_enter;
  intptr_t t_code_begin_offset =
      (intptr_t)microprofile_tramp_code_begin - t_enter;
  intptr_t t_code_end_offset = (intptr_t)microprofile_tramp_code_end - t_enter;
  intptr_t t_code_intercept0_offset =
      (intptr_t)microprofile_tramp_intercept0 - t_enter;
  ...
  intptr_t t_code_call_patch_push_offset =
      (intptr_t)microprofile_tramp_call_patch_push - t_enter;
  intptr_t t_code_call_patch_pop_offset =
      (intptr_t)microprofile_tramp_call_patch_pop - t_enter;
  ...
  int t_trunk_size = (int)((intptr_t)microprofile_tramp_end -
                           (intptr_t)microprofile_tramp_trunk);
  ...
  if (!MicroProfileCopyInstructionBytes(
          pInstructionMoveDest, f, nBytesToCopy, (int)codemaxsize, pTrunk,
          t_trunk_size, nUsableJumpRegs, &nInstructionBytesDest,
          &nInstructionBytesSrc, &nRegsWritten, &nRetSafe)) {
    if (pError) {
      const char *pCode = (const char *)f;
      memset(pError->Code, 0, sizeof(pError->Code));
      memcpy(pError->Code, pCode, nInstructionBytesSrc);
      int off =
          stbsp_snprintf(pError->Message, sizeof(pError->Message),
                         "Failed to move %d code bytes ", nInstructionBytesSrc);
      pError->nCodeSize = nInstructionBytesSrc;
      for (int i = 0; i < nInstructionBytesSrc; ++i) {
        off +=
            stbsp_snprintf(off + pError->Message, sizeof(pError->Message) - off,
                           "%02x ", 0xff & pCode[i]);
      }
      uprintf("%s\n", pError->Message);
    }
  ...
  if (0 == reg) {
    ...
    MicroProfileInsertRetJump(pInstructionMoveDest + nInstructionBytesDest,
                              phome);
  } else {
    int r = R_RAX;
    while ((reg & 1) == 0) {
    ...
    MicroProfileInsertRegisterJump(pInstructionMoveDest + nInstructionBytesDest,
                                   phome, r);
  }

  // PATCH 1 TRAMP EXIT
  intptr_t microprofile_tramp_exit = (intptr_t)ptramp + t_code_exit_offset;
  memcpy(ptramp + t_enter_patch_offset + 2, (void *)&microprofile_tramp_exit,
         8);
  ...
  memcpy((char *)ptramp + t_code_call_patch_push_offset + 2, &patch_push_addr,
         8);
...
static void MicroProfileMakeWriteable(void *p_, size_t s, DWORD *oldFlags) {
  ...
  for (uint32_t i = 0; i < nNumPages; ++i) {
    if (!VirtualProtect((void *)(aligned + nPageSize * i), nPageSize,
                        PAGE_EXECUTE_READWRITE, oldFlags + i)) {
      MP_BREAK();
    }
  }
...
void *MicroProfileAllocExecutableMemoryUp(intptr_t nBase, size_t s) {
  ...
  while (nBase < nEnd) {
    ...
    if (mbi.State == MEM_FREE && mbi.RegionSize >= s) {
      void *pMemory = VirtualAlloc((void *)nBase, s, MEM_COMMIT | MEM_RESERVE,
                                   PAGE_READWRITE);
      if (pMemory) {
        return pMemory;
      }
...
int MicroProfileTrimFunctionName(const char *pStr, char *pOutBegin,
                                 char *pOutEnd) {
...
BOOL CALLBACK MicroProfileEnumModules(_In_ PCTSTR ModuleName,
                                      _In_ DWORD64 BaseOfDll,
                                      _In_opt_ PVOID UserContext) {

  MODULEINFO MI;
  GetModuleInformation(GetCurrentProcess(), (HMODULE)BaseOfDll, &MI,
                       sizeof(MI));
  MEMORY_BASIC_INFORMATION B;
  int r = VirtualQuery((LPCVOID)BaseOfDll, (MEMORY_BASIC_INFORMATION *)&B,
                       sizeof(B));
  char buffer[1024];
  int r1 = GetLastError();
  if (r == 0) {
  ...
  MicroProfileSymbolInitModule(ModuleName, BaseOfDll,
                               BaseOfDll + MI.SizeOfImage);
  return true;
}
...
struct QueryCallbackBase // fucking c++, this is a pain in the ass
{
  virtual void CB(const char *pName, const char *pShortName, intptr_t addr,
                  intptr_t addrend, uint32_t nModuleId) = 0;
};
template <typename T> struct QueryCallbackImpl : public QueryCallbackBase {
  T t;
  QueryCallbackImpl(T t) : t(t) {}
  virtual void CB(const char *pName, const char *pShortName, intptr_t addr,
                  intptr_t addrend, uint32_t nModuleId) {
...
BOOL MicroProfileQueryContextEnumSymbols(_In_ PSYMBOL_INFO pSymInfo,
                                         _In_ ULONG SymbolSize,
                                         _In_opt_ PVOID UserContext) {
  uint32_t nModuleId = nLastModuleIdWin32;
  if (nLastModuleBaseWin32 != (intptr_t)pSymInfo->ModBase) {
    nLastModuleIdWin32 = nModuleId = MicroProfileSymbolGetModule(
        (const char *)(intptr_t)-2, pSymInfo->ModBase);
  ...
  if (pSymInfo->Tag == 5 || pSymInfo->Tag == 10) {
    ...
    int ret = 0;
    int l = MicroProfileTrimFunctionName(pSymInfo->Name, &FunctionName[0],
                                         &FunctionName[1024]);
    QueryCallbackBase *pCB = (QueryCallbackBase *)UserContext;

    pCB->CB(pSymInfo->Name, l ? &FunctionName[0] : 0,
            (intptr_t)pSymInfo->Address,
            pSymInfo->Size + (intptr_t)pSymInfo->Address, nModuleId);
  }
  return TRUE;
};
...
void MicroProfileIterateSymbols(Callback CB, uint32_t *nModules,
  ...
  if (MicroProfileSymInit()) {
    ...
    if (SymEnumerateModules64(
            GetCurrentProcess(),
            (PSYM_ENUMMODULES_CALLBACK64)MicroProfileEnumModules, NULL)) {
    }
    ...
    if (nNumModules) {
      ...
      for (uint32_t i = 0; i < nNumModules; ++i) {
        ...
        for (int j = 0; j < S.SymbolModules[nModule].nNumExecutableRegions;
             ++j) {
          ...
          while (b < e) {
            ...
            case PAGE_EXECUTE_WRITECOPY:
              ...
              // (void*)(intptr_t(B.BaseAddress) + B.RegionSize), B.RegionSize /
              // 1024.f, B.State, B.Protect);
            }
        ...
        // S.SymbolModules[nModule].nAddrBegin, (const
        // char*)S.SymbolModules[nModule].pBaseString);
        for (int j = 0; j < S.SymbolModules[nModule].nNumExecutableRegions;
             ++j) {
          if (SymEnumSymbols(GetCurrentProcess(),
                             S.SymbolModules[nModule].Regions[j].nBegin, "*",
                             MicroProfileQueryContextEnumSymbols, pBase)) {
      ...
      if (SymEnumSymbols(GetCurrentProcess(), 0, "*!*",
                         MicroProfileQueryContextEnumSymbols, pBase)) {
        uprintf("SymEnumSymbols Succeeds!\n");
      }
    }
...
BOOL MicroProfileQueryContextEnumSymbols1(_In_ PSYMBOL_INFO pSymInfo,
                                          _In_ ULONG SymbolSize,
                                          _In_opt_ PVOID UserContext) {
  if (pSymInfo->Tag == 5 || pSymInfo->Tag == 10) {
    char str[200];
...
const char *MicroProfileDemangleSymbol(const char *pSymbol) {
  return pSymbol; // todo: for some reasons all symbols im seaing right now are
                  // already undecorated?
}
...
void MicroProfileInstrumentWithoutSymbols(const char **pModules,
  ...
  if (MicroProfileSymInit()) {
    ...
    for (uint32_t i = 0; i < nNumSymbols; ++i) {
      int nCount = stbsp_snprintf(SymString, sizeof(SymString) - 1, "%s!%s",
                                  pModules[i], pSymbols[i]);
      if (nCount <= sizeof(SymString) - 1) {
        g_pFunctionFoundHack = 0;
        if (SymEnumSymbols(h, 0, SymString,
                           MicroProfileQueryContextEnumSymbols1, 0)) {
          if (g_pFunctionFoundHack) {
            uint32_t nColor = MicroProfileColorFromString(pSymbols[i]);
            const char *pDemangled =
                pSymbols[i]; // MicroProfileDemangleSymbol(pSymbols[i]);
            MicroProfileInstrumentFunction(g_pFunctionFoundHack, pModules[i],
                                           pDemangled, nColor);
          }
        }
      }
...
void MicroProfileSymbolUpdateModuleList() {
  MICROPROFILE_SCOPEI("MicroProfile", "MicroProfileSymbolUpdateModuleList",
                      MP_PINK3);
  //	QueryCallbackImpl<Callback> Context(CB);
  if (MicroProfileSymInit()) {
    uprintf("symbols loaded!\n");
...
bool MicroProfilePatchFunction(void *f, int Argument,
  ...
  intptr_t t_end_offset = (intptr_t)microprofile_tramp_end - t_enter;
  intptr_t t_trunk_offset = (intptr_t)microprofile_tramp_trunk - t_enter;
  intptr_t t_trunk_size =
      (intptr_t)microprofile_tramp_end - (intptr_t)microprofile_tramp_trunk;
  ...
  uint32_t nRegsWritten = 0;
  uint32_t nRetSafe = 0;
  uint32_t nUsableJumpRegs = (1 << R_RAX) | (1 << R_R10) |
                             (1 << R_R11); // scratch && !parameter register
  if (!MicroProfileCopyInstructionBytes(
          pInstructionMoveDest, f, nBytesToCopy, codemaxsize, pTrunk,
          t_trunk_size, nUsableJumpRegs, &nInstructionBytesDest,
          &nInstructionBytesSrc, &nRegsWritten, &nRetSafe)) {
    if (pError) {
      const char *pCode = (const char *)f;
...
static void MicroProfileMakeWriteable(void *p_) {
  ...
  while (KERN_SUCCESS == (kr = mach_vm_region_recurse(
                              task, &vmoffset, &vmsize, &nd,
    ...
    if (p >= (intptr_t)vmoffset && p <= intptr_t(vmoffset + vmsize)) {
      if (0 == (vbr.protection & VM_PROT_WRITE)) {
        ...
        int x = mprotect((void *)vmoffset, vmsize,
                         PROT_WRITE | PROT_READ | PROT_EXEC);
        if (x) {
          // uprintf("mprotect failed ... err %d:: %d   %s\n", errno, x,
          // strerror(errno));
        } else {
          uprintf("region is [%llx,%llx] .. %08llx  %d", vmoffset,
                  vmoffset + vmsize, vmsize, vbr.is_submap);
...
const char *MicroProfileDemangleSymbol(const char *pSymbol) {
  ...
  static char *pTempBuffer = (char *)malloc(
      size); // needs to be malloc because demangle function might realloc it.
...
void MicroProfileIterateSymbols(Callback CB, uint32_t *nModules,
  ...
  auto OnFunction = [&](void *addr, void *addrend, const char *pSymbol,
                        const char *pModuleName, void *pModuleAddr) -> bool {
    const char *pStr = MicroProfileDemangleSymbol(pSymbol);
    ;
    int l = MicroProfileTrimFunctionName(pStr, &FunctionName[0],
                                         &FunctionName[1024]);
    if (nCurrentModule != (intptr_t)pModuleAddr) {
      nCurrentModule = (intptr_t)pModuleAddr;
      nCurrentModuleId =
          MicroProfileSymbolGetModule(pModuleName, nCurrentModule);
    }

    CB(l ? &FunctionName[0] : pStr, l ? &FunctionName[0] : 0, (intptr_t)addr,
       (intptr_t)addrend, nCurrentModuleId);
    return true;
  };
  ...
  while (KERN_SUCCESS == (kr = mach_vm_region_recurse(
                              ...
                              (vm_region_recurse_info_t)&vbr, &vbrcount))) {
    {
      ...
      if (0 != (vbr.protection & VM_PROT_EXECUTE)) {
        ...
        if (bProcessModule) {
          S.SymbolModules[nModule].nProgressTarget =
              S.SymbolModules[nModule].Regions[0].nEnd -
              S.SymbolModules[nModule].Regions[0].nBegin;
          dl_info di;
          int r = 0;
      ...
      if (0 != (vbr.protection & VM_PROT_EXECUTE)) {
        ...
        if (r) {
          uprintf("[0x%p-0x%p] (0x%p) %s %s\n", (void *)vmoffset,
                  (void *)addr_prev, di.dli_fbase, di.dli_fname, di.dli_sname);
          MicroProfileSymbolInitModule(di.dli_fname, (intptr_t)vmoffset,
                                       (intptr_t)vmoffset + vmsize);
        }
      }
    }
    vmoffset += vmsize;
...
static void *MicroProfileAllocExecutableMemory(void *f, size_t s) {
  ...
  void *pMem = mmap((void *)f, s, PROT_READ | PROT_WRITE | PROT_EXEC,
...
extern "C" void
microprofile_tramp_enter_patch() asm("_microprofile_tramp_enter_patch");
extern "C" void microprofile_tramp_enter() asm("_microprofile_tramp_enter");
extern "C" void
microprofile_tramp_code_begin() asm("_microprofile_tramp_code_begin");
extern "C" void
microprofile_tramp_code_end() asm("_microprofile_tramp_code_end");
extern "C" void
microprofile_tramp_intercept0() asm("_microprofile_tramp_intercept0");
...
extern "C" void
microprofile_tramp_call_patch_pop() asm("_microprofile_tramp_call_patch_pop");
extern "C" void
microprofile_tramp_call_patch_push() asm("_microprofile_tramp_call_patch_push");
    ...
    if (r) {
      ...
      for (int j = 0; j < M.nNumExecutableRegions; ++j) {
        ...
        while (1) {
          ...
          if (r && di.dli_sname) {
            OnFunction(di.dli_saddr, (void *)nAddrPrev, di.dli_sname,
                       di.dli_fname, di.dli_fbase);
            nAddrPrev = (intptr_t)di.dli_saddr;
            nAddr = (intptr_t)di.dli_saddr - 1;
          } else {
            nAddr =
                (nAddr - 7) & ~7; // pretty ineffecient, but it seems linux just
                                  // returns 0 when there is no symbols, making
                                  // this the only option I can come up with?
          }
          if (nAddr < nBegin) {
            break;
          }
...
void MicroProfileSymbolUpdateModuleList() {
  ...
  while ((read = getline(&line, &len, F)) != -1) {
    ...
    if (8 == sscanf(line, "%p%c%p%c%c%c%c%c", &pBase, &c, &pEnd, &c, &r, &w, &x,
                    &p)) {
      if ('x' == x) {
        int r = 0;
        r = dladdr(pBase, &di);
        if (r) {
          if ('[' != di.dli_fname[0]) {
            MicroProfileSymbolInitModule(di.dli_fname, (intptr_t)pBase,
                                         (intptr_t)pEnd);
          }
        }
      }
...
static void *MicroProfileAllocExecutableMemory(void *f, size_t s) {
  ...
  void *pMem = mmap(f, s, PROT_READ | PROT_WRITE | PROT_EXEC,
                    MAP_ANON | MAP_PRIVATE, 0, 0);
  return pMem;
}
...
void MicroProfileHashTableInit(MicroProfileHashTable *pTable,
                               uint32_t nInitialSize, uint32_t nSearchLimit,
                               MicroProfileHashCompareFunction CompareFunc,
                               MicroProfileHashFunction HashFunc) {
...
void MicroProfileHashTableGrow(MicroProfileHashTable *pTable) {
  ...
  MicroProfileHashTableInit(&New, nNewSize, pTable->nSearchLimit,
                            pTable->CompareFunc, pTable->HashFunc);
  ...
  while (1) {
    ...
    for (uint32_t i = 0; i < pTable->nAllocated; ++i) {
      ...
      if (pEntries[Idx].Hash == 0) {
        ...
        return true;
      } else if (pEntries[Idx].Hash == H && (Cmp ? (Cmp)(Key, pEntries[Idx].Key)
                                                 : Key == pEntries[Idx].Key)) {
        pEntries[Idx].Value = Value;
        return true;
      } else if (i > nLim) {
        break;
...
void MicroProfileStringBlockFree(MicroProfileStringBlock *pBlock) {
  ...
  MicroProfileCounterAdd(
      S.CounterToken_StringBlock_Memory,
      -(int64_t)(pBlock->nSize + sizeof(MicroProfileStringBlock)));

  MP_FREE(pBlock);
}
MicroProfileStringBlock *MicroProfileStringBlockAlloc(uint32_t nSize) {
  nSize =
      MicroProfileMax(nSize, (uint32_t)(MicroProfileStringBlock::DEFAULT_SIZE -
                                        sizeof(MicroProfileStringBlock)));
  ...
  // (int)sizeof(MicroProfileStringBlock));
  MicroProfileStringBlock *pBlock =
      (MicroProfileStringBlock *)MP_ALLOC(nSize, 8);
  pBlock->pNext = 0;
...
void MicroProfileStringsInit(MicroProfileStrings *pStrings) {
  MicroProfileHashTableInit(&pStrings->HashTable, 1, 25,
                            MicroProfileHashTableCompareString,
                            MicroProfileHashTableHashString);
  pStrings->pFirst = 0;
  pStrings->pLast = 0;
}
void MicroProfileStringsDestroy(MicroProfileStrings *pStrings) {
...
const char *MicroProfileStringInternLower(const char *pStr) {
  return MicroProfileStringIntern(pStr, (uint32_t)strlen(pStr),
                                  ESTRINGINTERN_LOWERCASE);
}

const char *MicroProfileStringIntern(const char *pStr_, uint32_t nLen,
                                     uint32_t nFlags) {
  ...
  if (MicroProfileHashTableGetString(&pStrings->HashTable, pStr, &pRet)) {
    ...
    if (0 == pBlock || pBlock->nUsed + nLen > pBlock->nSize) {
      ...
      if (pBlock) {
        ...
        MicroProfileCounterAdd(S.CounterToken_StringBlock_Waste,
                               pBlock->nSize - pBlock->nUsed);
      } else {
  ...
  for (uint32_t i = 0; i < pTable->nAllocated; ++i) {
    if (pTable->pEntries[i].Hash != 0) {
      uprintf("[%05d,%05" PRIu64 "]  ::::%" PRIx64 ", %p .. hash %" PRIx64 "\n",
              i, pTable->pEntries[i].Hash % pTable->nAllocated,
              pTable->pEntries[i].Key, (void *)pTable->pEntries[i].Value,
  ...
  for (uint32_t i = 0; i < pTable->nAllocated; ++i) {
    if (pTable->pEntries[i].Hash != 0) {
      uprintf("%03d [%05d,%05" PRIu64 "]  ::::%s, %s .. hash %" PRIx64 "\n",
              c++, i, pTable->pEntries[i].Hash % pTable->nAllocated,
...
static const char *txt[] = {
    "gaudy",     "chilly",     "obtain",     "suspend",       "jelly",
    "peel",      "nauseating", "complain",   "cave",          "practise",
    "sail",      "close",      "drawer",     "mature",        "impossible",
    "exist",     "sister",     "poke",       "ancient",       "paddle",
    "ask",       "shallow",    "outrageous", "healthy",       "reading",
    "obey",      "water",      "elbow",      "abnormal",      "trap",
    "wholesale", "lovely",     "stupid",     "comparison",    "swim",
    "brash",     "towering",   "accept",     "invention",     "plantation",
    "spooky",    "tiger",      "knot",       "literate",      "awake",
    "itch",      "medical",    "ticket",     "tawdry",        "correct",
    "mine",      "accidental", "dinner",     "produce",       "protective",
    "red",       "dreary",     "toe",        "drain",         "zesty",
    "inform",    "boundless",  "ghost",      "attend",        "rely",
    "fill",      "liquid",     "pump",       "continue",      "spark",
    "church",    "fortunate",  "truthful",   "conscious",     "possible",
    "motion",    "evanescent", "branch",     "skirt",         "number",
    "meek",      "hour",       "form",       "work",          "car",
    "post",      "talk",       "fear",       "tightfisted",   "dress",
    "perform",   "fry",        "courageous", "dysfunctional", "page",
    "one",       "annoy",      "abrasive",   "dependent",     "payment"};
...
#define NUM_ITEMS 100
  ...
  MicroProfileHashTableInit(&Strings, 1, 25, MicroProfileHashTableCompareString,
                            MicroProfileHashTableHashString);
  uint32_t nCount = sizeof(txt) / sizeof(txt[0]);
